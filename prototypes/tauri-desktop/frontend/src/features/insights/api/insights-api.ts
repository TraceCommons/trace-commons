...
export async function getInsightsData(): Promise<InsightsData> {
  const [listResponse, summaryResponse] = await Promise.all([
    invokeTauri<unknown>("insights_list"),
    invokeTauri<unknown>("insights_summary"),
  ]);
...
export async function analyzeInsight(
  ...
  const response = record(
    await invokeTauri<unknown>("analyze_insight", {
      source,
      save,
      fileBytes: bytes,
    }),
...
export async function explainInsight(id: string) {
  const response = record(
    await invokeTauri<unknown>("explain_insight", { id }),
    "explain",
  );
  return parseInsight(response.insight);
}
...
export async function deleteInsight(id: string) {
  const response = record(
    await invokeTauri<unknown>("delete_insight", { id }),
    "delete",
  );
...
export async function annotateInsight(
  ...
  const response = record(
    await invokeTauri<unknown>("annotate_insight", { id, category, outcome }),
    "assessment",
  );
  return parseInsight(response.insight);
}
...
export async function clearInsightAnnotation(id: string) {
  const response = record(
    await invokeTauri<unknown>("clear_insight_annotation", { id }),
    "assessment",
  );
  return parseInsight(response.insight);
}
...
export async function linkTestReport(id: string, file: File) {
  const bytes = Array.from(new Uint8Array(await file.arrayBuffer()));
  const response = record(
    await invokeTauri<unknown>("link_test_report", { id, fileBytes: bytes }),
...
export async function linkGit(id: string, repository: string, commit: string) {
  const response = record(
    await invokeTauri<unknown>("link_git_insight", { id, repository, commit }),
    "Git evidence",
  );
  return parseInsight(response.insight);
}
...
export async function unlinkEvidence(id: string, evidenceId: string) {
  const response = record(
    await invokeTauri<unknown>("unlink_insight_evidence", {
      id,
      evidenceId,
    }),
    "unlink evidence",

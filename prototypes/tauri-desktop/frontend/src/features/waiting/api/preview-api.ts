...
export async function getPreviewBodyPage(
  ...
  const item = record(
    await invokeTauri<unknown>("preview_body", {
      entryId,
      offset,
      limit: 256 * 1024,
...
export async function getPreviewTurns(
  ...
  const item = record(
    await invokeTauri<unknown>("preview_turns", { entryId, bodyDigest }),
    "turns",
  );
...
export async function searchOriginal(entryId: string, needle: string) {
  const item = record(
    await invokeTauri<unknown>("search_original", { entryId, needle }),
    "search",
  );

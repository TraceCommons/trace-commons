...
export async function validatePublicRunEditor(
  ...
  const value = record(
    await invokeTauri<unknown>("validate_public_run_editor", input),
    "validation",
  );
...
export async function publishPublicRun(
  ...
  return parsePage(
    await invokeTauri<unknown>("publish_public_run", {
      submissionId,
      draft,
      taskSuccess,
      contributedVersion,
...
export async function unpublishPublicRun(submissionId: string) {
  const value = record(
    await invokeTauri<unknown>("unpublish_public_run", { submissionId }),
    "unpublish",
  );

...
mod calibrate_impl {
    ...
    // Env vars: same names as in tracedao-ingest.rs. We re-declare them
...
async fn run_bakeoff(args: BakeOffArgs) -> anyhow::Result<()> {
    ...
    let mock_perplexity = if args.mock_scorer && args.scorer.use_perplexity() {
        Some(tracedao_gate_enclave::perplexity::MockPerplexityScorer::new())
    } else {
        None
    };
    let mock_token_rarity = if args.mock_scorer && args.scorer.use_token_rarity() {
        Some(tracedao_gate_enclave::perplexity::MockTokenRarityScorer::new())
    } else {
        None
    };
            ...
            if args.mock_scorer {
                ...
                let eval_scorers = run_candidate_eval::EvalScorers {
                    perplexity: mock_perplexity
                        .as_ref()
                        .map(|s| s as &dyn tracedao_gate_enclave::perplexity::PerplexityScorer),
                    token_rarity: mock_token_rarity
                        .as_ref()
                        .map(|s| s as &dyn tracedao_gate_enclave::perplexity::TokenRarityScorer),
...
mod tests {
    ...
    mod calibrate_legacy {
        ...
        use serde::Serialize;
        use tracedao_gate_enclave::embedder::MockEmbedder;
        use tracedao_gate_enclave::perplexity::MockPerplexityScorer;
        use tracedao_gate_enclave::vector_index::MockVectorIndex;

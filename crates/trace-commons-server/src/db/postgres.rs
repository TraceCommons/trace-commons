...
impl Database for PgBackend {
    ...
    async fn run_migrations(&self) -> Result<(), DatabaseError> {
        ...
        // V42 retrofits V36's table-wide SELECT grants to the V38 column-scoped
        // convention. Safe to apply after V36+; the USING(true) policies stay.
        let already_applied = client
            .query_opt(
                "SELECT 1 FROM _trace_commons_migrations WHERE version = $1",
                &[&42_i32],
        ...
        if !already_applied {
            client
                .batch_execute(include_str!(
                    "../../../../migrations/V42__trace_gate_driver_column_grants.sql"
                ))
                .await?;
            client
                .execute(
                    "INSERT INTO _trace_commons_migrations (version, name) VALUES ($1, $2)",
                    &[&42_i32, &"trace_gate_driver_column_grants"],
                )
                .await?;
        }
        Ok(())

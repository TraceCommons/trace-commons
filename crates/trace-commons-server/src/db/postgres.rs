                ...
                "SELECT
                 ...
                 LEFT JOIN LATERAL (
                    SELECT
                        true AS has_policy,
                        pol.cmd = '*'
                            AND pg_get_expr(pol.qual, pol.polrelid) = '(tenant_id = current_setting(''trace-commons.trace_tenant_id''::text, true))'
                            AND pg_get_expr(pol.with_check, pol.polrelid) = '(tenant_id = current_setting(''trace-commons.trace_tenant_id''::text, true))'

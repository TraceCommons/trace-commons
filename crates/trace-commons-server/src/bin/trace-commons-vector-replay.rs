...
const HELP_TEXT: &str = "trace-commons-vector-replay
...
Usage:
  trace-commons-vector-replay --tenant-id <uuid>
                         [--fresh | --incremental]
...
fn init_tracing() {
    let filter = std::env::var("RUST_LOG").unwrap_or_else(|_| {
        "trace_commons_vector_replay=info,trace_commons_server=info,trace_commons_gate_enclave=info".into()
    });
...
async fn run_replay(args: ReplayArgs) -> Result<ReplaySummary> {
    ...
    // `trace-commons-ingest` partitions tenants by uuid in `trace_tenants`);
    ...
    // Build the KEK wrapper. Mirrors `trace-commons-ingest`'s
...
where
    P: trace_commons_server::trace_artifact_store::RemoteTraceArtifactProvider,
{

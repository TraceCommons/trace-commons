...
//! `TRACE_COMMONS_PERPLEXITY_INTEGRATION=1` and `TRACE_COMMONS_PERPLEXITY_MODEL_PATH`
...
mod candle_impl {
    ...
    impl CandlePerplexityScorer {
        ...
        /// forward. Callers in `trace-commons-server` translate their
...
mod tests {
    ...
    fn candle_perplexity_smoke_against_real_model() {
        ...
        if std::env::var("TRACE_COMMONS_PERPLEXITY_INTEGRATION").ok().as_deref() != Some("1") {
            eprintln!(

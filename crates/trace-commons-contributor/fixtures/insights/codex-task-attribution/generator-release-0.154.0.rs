...
const OUTPUT_DIR: &str = "/private/tmp/trace-insights-codex-release-writer-fixtures";
...
async fn writes_tool_reasoning_profile_fixture() -> Result<()> {
    ...
    mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ev_response_created("response-tool-1"),
                ev_reasoning_item("reasoning-tool-1", &["synthetic reasoning summary"], &[]),
                ev_function_call(
                    "call-tool-1",
                    "exec_command",
                    r#"{"cmd":"printf synthetic-tool-output"}"#,
                ),
                ev_completed("response-tool-1"),
            ]),
            sse(vec![
                ev_response_created("response-tool-2"),
                ev_assistant_message("message-tool-1", "synthetic tool result"),
                ev_completed("response-tool-2"),
            ]),
        ],
    )
    .await;
    let test = test_codex()
        .with_model("model-tool")
        .with_config(|config| {
            ...
            config
                ...
                .expect("set synthetic fixture sandbox policy");
        })
        .build(&server)
        .await?;
    test.codex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
            text: "synthetic tool refactor turn".into(),
            text_elements: Vec::new(),
        }]))
        .await?;
    wait_for_event(&test.codex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    let raw = std::fs::read_to_string(&rollout_path)?;
    anyhow::ensure!(
        raw.contains("Process exited with code 0") && raw.contains("synthetic-tool-output"),
        "synthetic tool did not complete successfully"
    );
    let sanitized = sanitize_rollout(&raw, "tool")?;
    std::fs::write(output_dir.join("codex-tool-reasoning.jsonl"), sanitized)?;
    test.codex.shutdown_and_wait().await?;
    Ok(())
}
...
async fn writes_two_direct_profile_fixtures() -> Result<()> {
    ...
    let manifest = serde_json::json!({
        ...
        "provenance": {
            ...
            "build_qualification_limit": "Generated from the pinned rust-v0.154.0 release source with synthetic localhost responses. Structural qualification only; no live provider identity or outcome claim.",
    ...
    fn sanitize(&mut self, value: &mut Value, key: Option<&str>, parent: Option<&str>) {
        match value {
            ...
            Value::String(text) => match key {
                ...
                Some("filesystem") if parent == Some("environments") => {
                    // The released writer embeds workspace roots in this XML string.
                    // Use the same identity map as structured roots, preserving any
                    // distinct roots rather than collapsing them to the cwd.
                    let original = text.clone();
                    for tail in original.split("<root>").skip(1) {
                        if let Some((source, _)) = tail.split_once("</root>") {
                            if source.starts_with('/') {
                                let next = format!("/w/path-{}", self.paths.len() + 1);
                                let replacement = self.paths.entry(source.to_string())
                ...
                Some("output") if parent == Some("payload") => {

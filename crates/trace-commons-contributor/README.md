...
`daemon status` distinguishes a running daemon from an offline snapshot of the state its next start will load. Its JSON result includes `daemon_running`. When no daemon is running, `daemon pause` and `daemon resume` report that the change is recorded for the next start.

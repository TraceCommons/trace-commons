        ...
        Ok(summary) => {
            Response::ok(
                ...
                serde_json::json!({
                    ...
                    // The configuration fingerprint -- cheap, a hash of the
                    // config rather than of the envelope -- so it still
                    // rides along here. There is no `envelope_digest` in
                    // this response: this call never built one, and never
                    // pinned this entry. See the handler's doc comment.
                    ...
                    // build, and the fingerprint above is not bindable to a
                    // later approval.

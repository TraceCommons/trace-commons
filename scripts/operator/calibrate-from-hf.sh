...
# - Pipes through `trace-commons-gate-calibrate` (the new binary).
...
BIN=""
for candidate in \
  "$(pwd)/target/release/trace-commons-gate-calibrate" \
  "target/release/trace-commons-gate-calibrate" \
  "$(command -v trace-commons-gate-calibrate || true)"; do
  if [ -n "$candidate" ] && [ -x "$candidate" ]; then
    BIN="$candidate"
    break
  fi
done
[ -n "$BIN" ] || bail "trace_commons_gate_calibrate_binary_not_found"

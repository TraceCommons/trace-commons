...
docker run --rm --detach \
  --name "${CONTAINER}" \
  -e POSTGRES_PASSWORD=phase5-admin \
...
"${ROOT}/target/debug/trace-commons-pipeline-local" serve \
  ...
  --compatibility-policies >"${MIGRATION_LOG}" 2>&1 &
MIGRATION_PID=$!
...
docker exec "${CONTAINER}" psql -U postgres -v ON_ERROR_STOP=1 -c \
  "CREATE ROLE pipeline_runtime LOGIN PASSWORD 'phase5-runtime' NOBYPASSRLS;
...
"${ROOT}/target/debug/trace-commons-pipeline-local" serve \
  --database-url "postgres://pipeline_runtime:phase5-runtime@127.0.0.1:${PG_PORT}/postgres" \
  --bind "127.0.0.1:${PORT}" \
  --artifact-root "${ARTIFACT_ROOT}" \
  --allow-minimal-policies \

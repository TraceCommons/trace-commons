#!/usr/bin/env bash
#
# build-cuda-bakeoff.sh — reproducible CUDA build of tracedao-gate-calibrate
...
#   4. Builds tracedao-gate-calibrate with --features local-gpu-models-cuda
...
REPO_ROOT="${REPO_ROOT:-$HOME/work/tracedao-server}"
...
echo "=== building tracedao-gate-calibrate (CUDA) ==="
cd "$REPO_ROOT"
RUSTFLAGS="-C link-arg=${SHIM_O}" \
  ...
  cargo build --release \
    -p tracedao-server \
    --bin tracedao-gate-calibrate \
    --features local-gpu-models-cuda

echo ""
echo "OK: binary at $REPO_ROOT/target/release/tracedao-gate-calibrate"
ls -la "$REPO_ROOT/target/release/tracedao-gate-calibrate"

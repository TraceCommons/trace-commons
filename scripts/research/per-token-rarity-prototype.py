...
`crates/tracedao-server/src/bin/gate_calibrate/bakeoff_corpus.rs`), runs a
...
landing site for the metric is the `tracedao-gate-enclave::perplexity_local`
...
def discrimination_auc(novel: Sequence[float], duplicate: Sequence[float]) -> float:
    ...
    `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_metrics.rs` so the

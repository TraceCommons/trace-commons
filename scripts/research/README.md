...
as Rust code in `crates/tracedao-server` or `crates/tracedao-gate-enclave`.
...
1. Loads a bake-off corpus `.tar.zst` produced by the bake-off binary
   (`crates/tracedao-server/src/bin/gate_calibrate/bakeoff_corpus.rs`) and
...
4. Computes Mann–Whitney U discrimination AUC for novel-vs-duplicate against
   ...
   `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_metrics.rs::discrimination_auc`
...
spec; the production landing site is the `tracedao-gate-enclave::perplexity_local`

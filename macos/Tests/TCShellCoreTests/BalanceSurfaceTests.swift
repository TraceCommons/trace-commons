...
final class BalanceSurfaceTests: XCTestCase {
    ...
    ///
    /// This suite kept its own copy through the consolidation that collapsed
    /// the others. Its 106 keys were byte-identical in value to the shared
    /// fixture's -- verified, not assumed -- so it was a sixth copy carrying
    /// no information, and exactly the failure the shared one exists to
    /// prevent: five new properties were added to `PrivateInferenceCopy` and
    /// the shared fixture, every suite that reads it passed, and this one
    /// failed to decode at all. A new required property is one edit there.

...
struct ComparisonSpecificationsView: View {
    ...
    @State private var explanation: ComparisonSpecificationsModel.ResultConfirmation?
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            Picker(text("comparison_specification_stratum"), selection: Binding(
                ...
                ForEach(model.options) { option in
                    Text("\(option.taskIDs.count) \(text("comparison_specification_matching_tasks")) · \(option.stratum.language)")
                        .tag(option.id)
                }
            }
            ...
            if let selected = model.selected {
                ...
                if let result = model.result {
                    ...
                    Button(text("comparison_specification_explain_result")) {
                        explanation = model.resultConfirmation()
                    }
                    .confirmationDialog(text("comparison_specification_explain_confirm"),
                                        isPresented: explanationBinding) {
                        Button(text("comparison_specification_explain_result")) {
                            if let explanation { model.explain(explanation) }; explanation = nil
                        }
                    } message: { Text(result.audit_digest) }
                }
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 5) {
            ...
            ForEach(spec.cutoff_task_evidence) { evidence in
                Button { openTask(evidence.task_id) } label: {
                    Text("\(text("outcome_\(evidence.outcome.rawValue)")) · \(evidence.outcome_recorded_at.map(InsightsDate.label) ?? text("comparison_task_unknown"))")
                }
            }
            DisclosureGroup(text("comparison_task_advanced_evidence")) {
    ...
    private func resultView(_ result: DescriptiveComparisonResult) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            ForEach(result.cohorts) { cohort in
                VStack(alignment: .leading) {
                    ...
                    ForEach(DescriptiveOutcome.allCases, id: \.self) { outcome in
                        Text("\(text("outcome_\(outcome.rawValue)")): \(count(outcome, in: cohort.outcomes))")
                    }
            ...
            ForEach(result.excluded_tasks) { excluded in
                VStack(alignment: .leading) {
                    Button(text("comparison_task_open")) { openTask(excluded.task_id) }
    ...
    private var explanationBinding: Binding<Bool> {
        Binding(get: { explanation != nil }, set: { if !$0 { explanation = nil } })
    }
}

...
export async function getCredentialStatus() {
  return parseCredentialStatus(
    await invokeTauri("private_ai_credential_status"),
  );
}
export async function getBalance(): Promise<BalanceStatus> {
  const value = record(await invokeTauri("private_ai_balance"));
  return {
...
export async function startCredential(provider: string) {
  const value = record(
    await invokeTauri("start_private_ai_credential", { provider }),
  );
  return {
...
export async function cancelCredential() {
  return invokeTauriDiscardResult("cancel_private_ai_credential");
}

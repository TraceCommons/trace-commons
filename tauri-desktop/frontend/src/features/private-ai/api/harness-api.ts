...
export async function getHarnessList(): Promise<HarnessList> {
  const value = record(await invokeTauri("private_ai_harnesses"));
...
export async function planHarness(
  ...
  const value = record(await invokeTauri("plan_harness", { id, action }));
  const view = record(value.view);
  if (
...
export async function commitHarness(planId: string) {
  return invokeTauriDiscardResult("commit_harness", { planId });
}

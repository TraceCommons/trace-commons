...
async function getAttestationCopy(
  ...
  const request = invokeTauri("attestation_copy", {
    label: label ?? "",
    reason,
...
export async function getWaitingData(): Promise<WaitingData> {
  const response = asRecord(await daemonCall("list_pending"));
...
async function parsePreview(value: unknown): Promise<WaitingPreview> {
  ...
  if (
    ...
    typeof record.gate_statement !== "string" ||
  ...
  return {
    ...
    gate_statement: record.gate_statement,
...
export async function previewWaitingEntry(entryId: string) {
  return parsePreview(await invokeTauri("preview_entry", { entryId }));
}

export async function dismissWaitingEntry(entryId: string): Promise<void> {
  await invokeTauri("dismiss_entry", { entryId });
}

function parseApproval(value: unknown) {
  const response = asRecord(value);
  if (
    typeof response.approved !== "number" ||
    (response.hold_until !== null && typeof response.hold_until !== "string") ||
    typeof response.flagged !== "number" ||
    !Array.isArray(response.skipped) ||
    !response.skipped.every(
      (item) =>
        isRecord(item) &&
        typeof item.entry_id === "string" &&
        typeof item.reason_label === "string",
    )
  ) {
  ...
  return {
    ...
    hold_until: response.hold_until as string | null,
    flagged: response.flagged,
    redactions: numberMap(response.redactions, "redactions"),
    skipped: response.skipped as ApprovalResult["skipped"],
    ...(typeof response.excluded_ineligible === "number"
      ? { excluded_ineligible: response.excluded_ineligible }
      : {}),
...
export async function getQueueOutcomeCounts(): Promise<QueueOutcomeCounts> {
  const response = asRecord(await daemonCall("queue_outcome_counts"));
  const reasons = numberMap(response.reasons, "outcome counts");
  const lines = Object.fromEntries(
    await Promise.all(
      Object.keys(reasons).map(async (label) => {
        const value = await invokeTauri("queue_outcome_line", {
          label,
        });

...
export async function listEpisodes(): Promise<EpisodeListEntry[]> {
  const response = record(
    await invokeTauri("insights_episode_list"),
    "episode list",
  );
...
export async function explainEpisode(id: string): Promise<EpisodeDetail> {
  const response = record(
    await invokeTauri("insights_episode_explain", { id }),
    "episode detail",
  );
...
export async function createEpisode(snapshotIds: string[]): Promise<Episode> {
  const response = record(
    await invokeTauri("insights_episode_create", { snapshotIds }),
    "episode create",
  );
...
export async function annotateEpisode(
  ...
  const response = record(
    await invokeTauri("insights_episode_annotate", {
      id,
      expectedRevision,
      category,
      outcome,
...
export async function clearEpisodeAssessment(
  ...
  const response = record(
    await invokeTauri("insights_episode_clear_assessment", {
      id,
      expectedRevision,
    }),
    "episode assessment",
...
export async function getQuestionCards(
  ...
  const response = record(
    await invokeTauri("insights_question_cards", {
      questions: [
        "recorded_activity",
        "episode_outcomes",
        "observed_models",

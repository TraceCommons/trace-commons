...
export async function listComparisonTasks(): Promise<ComparisonTaskDetail[]> {
  const response = record(
    await invokeTauri("comparison_task_list"),
    "task list",
  );
...
export async function explainComparisonTask(
  ...
  const response = record(
    await invokeTauri("comparison_task_explain", { id }),
    "task detail",
  );
...
export async function createComparisonTask(
  ...
  const response = record(
    await invokeTauri("comparison_task_create", { episodeIds }),
    "task create",
  );
...
export async function replaceComparisonTaskEpisodes(
  ...
  const response = record(
    await invokeTauri("comparison_task_replace_episodes", {
      id,
      expectedRevision,
      episodeIds,
    }),
...
export async function setComparisonTaskContext(
  ...
  const response = record(
    await invokeTauri("comparison_task_set_context", {
      id,
      expectedRevision,
      context: input,
    }),
...
export async function setComparisonTaskOutcome(
  ...
  const response = record(
    await invokeTauri("comparison_task_set_outcome", {
      id,
      expectedRevision,
      value,
    }),
...
export async function clearComparisonTaskOutcome(
  ...
  const response = record(
    await invokeTauri("comparison_task_clear_outcome", {
      id,
      expectedRevision,
    }),
    "task outcome",
...
export async function reconfirmComparisonTask(
  ...
  const response = record(
    await invokeTauri("comparison_task_reconfirm", {
      id,
      expectedRevision,

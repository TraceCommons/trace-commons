...
export async function getWitnessStatus() {
  return parse(await invokeTauri("witness_status"));
}
export async function configureWitness(
  ...
  return parse(
    await invokeTauri("configure_witness", {
      url,
      signingAddress,
      measurements,
    }),
...
export async function clearWitness() {
  return parse(await invokeTauri("clear_witness"));
}

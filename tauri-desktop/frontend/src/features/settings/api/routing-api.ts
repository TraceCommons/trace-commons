...
export async function discoverRouting() {
  return parseDiscovery(await invokeTauri("discover_routing"));
}
export async function configureRouting(
  ...
  return invokeTauriDiscardResult("configure_routing", {
    enabled,
    port,
    tokenDir: tokenDir.trim() || null,
  });
}
export async function probeRouting(port: number, tokenDir: string) {
  return record(
    await invokeTauri("probe_routing", {
      port,
      tokenDir: tokenDir.trim() || null,
    }),
    "probe",
...
export async function probeRoutedTools(port: number, tokenDir: string) {
  return parseEvidence(
    await invokeTauri("probe_routed_tools", {
      port,
      tokenDir: tokenDir.trim() || null,
    }),
  );

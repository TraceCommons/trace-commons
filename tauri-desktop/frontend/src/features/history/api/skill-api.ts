...
export async function getSkillCopy(): Promise<SkillCopy> {
  const item = record(await invokeTauri("skill_learning_copy"), "copy");
  const keys = [
    "heading",
    "promise",
    "supported_family",
...
export async function getSkillCandidate(submissionId: string) {
  return candidate(await invokeTauri("skill_candidate", { submissionId }));
}
export async function reviewSkill(
  ...
  return review(
    await invokeTauri("skill_review", {
      candidateId,
      draft: draftValue,
      replacesReviewId,
    }),
...
export async function evaluateSkill(reviewId: string, skillSha256: string) {
  return report(await invokeTauri("skill_evaluate", { reviewId, skillSha256 }));
}
export async function planSkillInstall(evaluationId: string) {
  return installPlan(await invokeTauri("skill_install_plan", { evaluationId }));
}
export async function commitSkillInstall(plan: SkillInstallPlan) {
  return installed(
    await invokeTauri("skill_install_commit", {
      planId: plan.plan_id,
...
export async function getSkillInstallStatus(sourceSubmissionId: string) {
  const value = record(
    await invokeTauri("skill_install_status", { sourceSubmissionId }),
    "status",
  );
...
export async function rollbackSkill(installedSkill: InstalledSkill) {
  const value = record(
    await invokeTauri("skill_install_rollback", {

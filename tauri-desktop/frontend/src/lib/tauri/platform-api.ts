...
export async function getPlatformCapabilities() {
  return parsePlatformCapabilities(await invokeTauri("platform_capabilities"));
}

export function requestNotificationPermission() {
  return invokeTauriDiscardResult("request_notification_permission");
}

export function setStartAtLogin(enabled: boolean) {
  return invokeTauriDiscardResult("set_start_at_login", { enabled });
}

export function openSystemSettings(area: "notifications" | "login_items") {
  return invokeTauriVoid("open_system_settings", { area });
}

export function quitApp() {
  return invokeTauriVoid("quit_app");
}

export async function pickDirectory(
  purpose: "repository" | "source_root",
): Promise<string> {
  const value = await invokeTauri("pick_directory", { purpose });
...
export async function consumeDeepLink(): Promise<
  ...
  | null
> {
  const value = await invokeTauri("consume_deep_link");
  ...
  if (response.kind === "credential" && typeof response.provider === "string") {

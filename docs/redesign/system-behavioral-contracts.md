...
[`proposal.md`](./proposal.md) is authoritative for architecture. The public
...
If those documents conflict with the pipeline model, `proposal.md` controls
...
- Pipeline contracts come from `proposal.md`.
...
- A credit event MUST identify its Score outcome.
- If a batch finalizes credit, the Settle result MUST identify it by hash.
- If no batch finalizes credit, the batch reference MUST be absent.
...
### SCR-003: Credit units and decision

- Score MUST express credit as checked integer microcredits.
- One credit MUST equal 1,000,000 microcredits.
- Conversion MUST reject overflow and excess precision.
- The Score decision MUST state the awarded microcredits.
- A zero-credit result MUST remain distinct from an incomplete Score phase.

**Acceptance:** Exercise zero, maximum, overflow, negative source input, and
excess-precision conversion cases.

### SCR-004: Score persistence and credit event

- The Score outcome and next phase MUST commit atomically.
- A positive Score MUST create one eligible credit event.
- The event key MUST identify the tenant, run, and Score outcome.
- A retry MUST NOT create another event.
- An embedding needed by Settle MUST remain encrypted evidence.
- The Score outcome MUST store only its artifact hash.

**Acceptance:** Crash before and after the Score transaction commits. A zero
Score creates no event. A committed positive Score creates exactly one event
with its stable key.
...
- Index application and credit settlement MUST record independent progress.
- An index failure MUST NOT corrupt the eligible credit event.
- A credit hold MUST NOT alter the index decision.
- Settle MUST wait for all required internal operations.
- Settle MUST preserve retry information for incomplete operations.

**Acceptance:** Fail the index and credit paths independently. Recover each
path without repeating the other operation. No Settle outcome can exist until
all required operations complete.

### STL-004: Credit settlement integration

- Settlement MUST use existing eligible credit events.
- Settlement MUST preserve account-level batching.
- Settlement MUST preserve holds, caps, issuer approval, source-list approval,
  and duplicate-credit protection.
- Concurrent batches MUST NOT select the same event.
- The Settle decision MUST contain index membership and finalized
  microcredits.
- If a batch finalizes credit, the Settle decision MUST identify it by hash.
- If no batch finalizes credit, the batch reference MUST be absent.
- Raw account references MUST NOT appear in outcomes.

**Acceptance:** Run previews, approvals, holds, caps, concurrent settlement,
and duplicate source fixtures. Finalize several runs in one compatible batch.
Each applicable run must reference the same batch hash and its own amount.
...
- Each completed phase has one outcome.
- A positive Score has one eligible credit event.

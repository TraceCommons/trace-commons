...
| Ranking/model utility pipeline | Partial | A trusted offline utility-credit worker route and CLI helper append idempotent delayed credit for accepted traces using `regression_catch`, `training_utility`, or `ranking_utility` plus an external job/artifact reference. Ranking workers can write hash-only model versions, feature hashes, predictions, frontier/reviewer labels, and persisted calibration runs with aggregate quality gates, registered calibration-dataset binding, configurable label-source diversity, and per-source cohort error gates before model-derived credit is trusted; admins can stage `candidate` models freely, and the admin promotion endpoint activates candidates only when the requested model/policy/target-use/calibration-dataset run is promotable, the latest matching evidence is still promotable, and any configured calibration freshness window is satisfied. The dedicated ranking prediction-credit worker converts a positive active-model prediction settlement score into one idempotent `ranking_utility` event bound to `ranking_prediction:<uuid>` only while the model's matching calibration dataset remains promotable and fresh under `TRACE_COMMONS_RANKING_CALIBRATION_MAX_AGE_HOURS` when configured; the batch run endpoint lets schedulers scan stored predictions with bounded forward progress and idempotent retries. Ranking utility settlement requires the named model to be active with a latest promotable calibration run for the settlement policy/target use and the active model's registered calibration dataset, and only fresh prediction-bound ranking credit events can settle. Production still needs external benchmark invalidation and broader model-risk monitoring. |

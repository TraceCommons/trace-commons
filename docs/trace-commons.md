...
Retention legal holds are service-configured by policy ID, not by trace submitters. A trace envelope can suggest a retention policy, but only the authenticated service configuration decides whether that policy is under legal hold. Review and maintenance cross-check current server retention policy ids plus expiration timestamps against the server-derived retention class before reading trace bodies, skipping expiration, or purging.

...
noise is configured plus the max delta, without exposing
`TRACE_COMMONS_ANALYTICS_BROAD_RELEASE_NOISE_KEY` or any derived key material.
...
count and keyed count-noise settings. The response returns only safe aggregate
...
| Analytics summary | Partial | Aggregate counts by status/risk/tool/coverage exist, including content-free process-evaluation aggregates for evaluated trace count, labels, rubric ratings, and score bands. Process-evaluation workers can also attach idempotent hash-only ranking labels for ranking-allowed traces, while still blocking disallowed target uses before trace-body reads. Deployments can set a minimum cell-count threshold to suppress rare aggregate buckets before responses leave the service, analytics responses include privacy-budget accounting with released/suppressed cell counts, suppression status, broad-release blocker reasons, and keyed count-noise metadata, and `release_scope=broad` fails closed unless the privacy budget is broad-release ready and broad-release noise is configured. Operational summary, operational metrics, rollout-smoke, and `POST /v1/admin/analytics-release-drill` now expose safe analytics-release readiness and hash-only evidence without leaking the noise key or raw aggregate maps. Formal differential-privacy accounting remains future work before broad public publication claims. |

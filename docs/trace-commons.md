...
Set `TRACE_COMMONS_REQUIRE_DB_MIRROR_WRITES=true` during production cutover after DB dual-write parity checks pass. It requires `TRACE_COMMONS_DB_DUAL_WRITE=true` and makes submission, revocation, review decision, credit, utility attestation, credit settlement, credit hold, NEAR credit outbox, ranking evidence/calibration, replay export manifest, benchmark/ranker provenance, audit, and trace-content-read mirror failures return an internal error instead of silently continuing with file-only state. Submission, delayed-credit, credit settlement control-plane, ranking evidence/calibration, replay export manifest, benchmark provenance, benchmark lifecycle, and ranker provenance mirror failures also avoid publishing local file-side metadata/object, ledger, settlement/outbox JSONL records, ranking JSONL evidence, replay-manifest, artifact/provenance, or staged service-local encrypted artifact objects for the failed operation. If a final export audit mirror fails after export metadata was staged, required mode removes the replay/benchmark/ranker file artifacts and deletes the DB export manifest/items/object refs for that export. Replay, benchmark, and ranker export DB mirrors write manifest metadata, export artifact object refs, and manifest item snapshots through one backend transaction so an item/reference validation failure does not leave a partial DB export.
...
For credit settlement cutover, the same maintenance run also backfills and reconciles utility attestations, credit settlement batches, credit holds, and NEAR credit outbox rows/status. Dirty gaps or status/release drift in any of those families are promotion blockers when `TRACE_COMMONS_REQUIRE_DB_RECONCILIATION_CLEAN=true` is enabled.
...
| Ranking/model utility pipeline | Partial | A trusted offline utility-credit worker route and CLI helper append idempotent delayed credit for accepted traces using `regression_catch`, `training_utility`, or `ranking_utility` plus an external job/artifact reference. Ranking workers can write hash-only model versions, feature hashes, predictions, frontier/reviewer labels, and persisted calibration runs with aggregate quality gates before model-derived credit is trusted; admins can stage `candidate` models freely, and the admin promotion endpoint activates candidates only when the requested model/policy/target-use calibration is promotable, the latest model/policy calibration evidence is still promotable, and any configured calibration freshness window is satisfied. The dedicated ranking prediction-credit worker converts a positive active-model prediction settlement score into one idempotent `ranking_utility` event bound to `ranking_prediction:<uuid>` only while the model's matching calibration remains fresh under `TRACE_COMMONS_RANKING_CALIBRATION_MAX_AGE_HOURS` when configured; the batch run endpoint lets schedulers scan stored predictions with bounded forward progress and idempotent retries. Ranking utility settlement requires the named model to be active with a latest promotable calibration run for the settlement policy/target use, and only fresh prediction-bound ranking credit events can settle. Production still needs external benchmark invalidation and broader model-risk monitoring. |
...
settlement score, and appends a single idempotent `ranking_utility` event whose
external reference is `ranking_prediction:<uuid>`.
...
consuming the batch limit, applies the same active-model/calibration/source
checks as the single-prediction route, and reports checked, credited,
already-existing, ineligible, and still-pending counts so a retrying scheduler
can make bounded forward progress without double-crediting.

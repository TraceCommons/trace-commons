...
Ranking worker-run ledger rows are part of the ranking DB mirror and the
...
the run id, dry-run flag, filters, limits, checked/succeeded/skipped counts,
result refs, and machine-readable skip reason counts, but store only the reason
hash rather than raw operator notes.

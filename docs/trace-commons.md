...
`GET /v1/admin/config-status` also reports `ranking_min_label_count`, the
server-owned deployment floor for joined ranking prediction/label pairs. This
safe projection lets operators confirm workers cannot lower the sample-size
requirement below production credit policy.

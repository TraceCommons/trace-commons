...
still cannot. `TRACE_COMMONS_OBJECT_STORE_REQUIRE_VERSIONING=true` is the
separate cloud-cutover guard: startup fails unless the configured store also
reports object versioning and restore-after-delete support, and the current
local-service/filesystem-remote adapters report false until a provider with
real rollback semantics is compiled. The server-owned

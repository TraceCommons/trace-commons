...
credit-account hash. The response omits raw account refs, source event ids,
external refs, hold reason details, and trace bodies.
...
`GET /v1/admin/credit-risk-summary` gives admins a safe pre-issuance view of
pending, held, and over-cap credit grouped by account hash, so operators can
review capped or held accounts without exposing raw account refs, event ids,
external refs, or hold details.

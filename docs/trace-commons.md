...
The `analytics_release` smoke check is recorded manually through
`POST /v1/admin/rollout-smoke/evidence` after the broad analytics preflight has
been exercised with the configured minimum cell count and keyed count-noise
settings; operational summary and metrics expose only readiness booleans,
bounded max-delta metadata, and blocker reason codes.

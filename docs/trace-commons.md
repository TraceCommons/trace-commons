...
Set `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_URL` to point revocation-propagation worker-queue items at a trusted invalidator adapter. The request is hash-only: it contains the tenant storage ref, revocation propagation item id, source submission ref hash, queue surface, and queue key hash, but no raw trace body, contributor token, tenant id, or submission id. The adapter must return a canonical `sha256:` `evidence_hash`, which is stored on the completed propagation item and used by the revocation-effects drill. `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_BEARER_TOKEN` adds bearer auth and `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_TIMEOUT_MS` bounds the call; HTTP endpoints are allowed only for localhost/loopback during development. When the adapter is absent, the worker preserves the existing local hash-only completion marker so pilots can rehearse ledger completion before deploying external cache operations. Config-status exposes only whether the adapter is configured and the timeout.

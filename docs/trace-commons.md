...
| Analytics summary | Partial | Aggregate counts by status/risk/tool/coverage exist, including content-free process-evaluation aggregates for evaluated trace count, labels, rubric ratings, and score bands. Process-evaluation workers can also attach idempotent hash-only ranking labels for ranking-allowed traces, while still blocking disallowed target uses before trace-body reads. Deployments can set a minimum cell-count threshold to suppress rare aggregate buckets before responses leave the service, and analytics responses now include privacy-budget accounting with released/suppressed cell counts, suppression status, and broad-release blocker reasons. Production still needs differential-privacy/noise calibration and release governance if exposed beyond reviewer/admin operators. |

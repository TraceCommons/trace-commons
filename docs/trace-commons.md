...
visibility: export workers can run dedicated replay/ranker export and export-job
automation routes, and those routes reject ordinary reviewer tokens before
export-job or source checks while reviewer-compatible export surfaces remain
available for manual review. Benchmark workers
can use the reviewer-compatible benchmark conversion/lifecycle surfaces where
manual review remains intended, and can run dedicated worker routes for
conversion, deterministic evaluation batches, registry publication batches, and
registry outbox submit/confirm/status automation. Those dedicated benchmark
worker routes reject ordinary reviewer tokens before source, evaluator, or
registry adapter checks. Retention workers can run the dedicated
retention worker route or retention-scoped maintenance; reviewer tokens are not accepted on that
...
```bash
# Mirror corpus metadata into the configured TraceCommons PostgreSQL database.
TRACE_COMMONS_DB_DUAL_WRITE=true \
DATABASE_URL=postgres://trace-commons:trace-commons@localhost/trace-commons \
cargo run --bin trace-commons-ingest
...
but serving traffic should use a non-owner role without `BYPASSRLS`. Operators
can pin the expected serving identity with
`TRACE_COMMONS_POSTGRES_RUNTIME_ROLE_SHA256`; the server reports the current
runtime role hash and blocks readiness when the configured hash does not match.

`/v1/admin/config-status` also reports whether broad-release analytics count
noise and privacy accounting are configured, the count-noise max delta, the
epsilon charged per broad release, and the tenant max epsilon cap without
exposing `TRACE_COMMONS_ANALYTICS_BROAD_RELEASE_NOISE_KEY` or any derived key
material.
...
credential, or path values, and operational metrics export the same finite
...
aggregate policy/RLS/FORCE RLS counts, role-bypass and runtime table-owner
state, and compact blocker codes, and can append `postgres_rls_readiness`
evidence.
...
`POST /v1/admin/analytics-release-drill` runs the broad analytics release
preflight against the reviewer metadata view using the configured minimum cell
count, keyed count-noise settings, and optional epsilon ledger. The response
returns only safe aggregate privacy-budget fields, compact blocker codes,
noise/accounting status, and a sha256-prefixed evidence hash; it can append the
`analytics_release` rollout-smoke evidence row directly without exposing the
noise key, tenant token, raw trace data, or aggregate maps.
`POST /v1/admin/ranking/readiness-drill` wraps the active-model risk,
dataset-readiness, and ranking-credit readiness reports into one hash-only
...
`source_list_hash`, and canonical approval `evidence_hash`. Approval audit rows
mirror typed hash-only metadata for the policy version, source-list hash,
approval evidence hash, reason hash, and optional evidence-ref hash, and the DB
mirror validates the metadata against the canonical reason fields before write
or backfill. The drill accepts `source_event_limit` with the same 1..=500 bounds used by live settlement
...
issuer profile missing-control names, and a sha256-prefixed evidence hash, and
can append `credit_settlement` rollout-smoke evidence directly. When
...
runtime logs and audit rows: completion summaries include only tenant refs,
purpose hashes, and counts, and per-row failures include only the outbox id plus
a hash of the sanitized error. Manual NEAR outbox status changes also append
...
Set `TRACE_COMMONS_REQUIRE_DB_MIRROR_WRITES=true` during production cutover after DB dual-write parity checks pass. It requires `TRACE_COMMONS_DB_DUAL_WRITE=true` and makes submission, revocation, review decision, credit, utility attestation, credit settlement, credit hold, NEAR credit outbox, benchmark registry outbox, ranking evidence/calibration, replay export manifest, benchmark/ranker provenance, audit, and trace-content-read mirror failures return an internal error instead of silently continuing with file-only state. Submission, delayed-credit, credit settlement control-plane, benchmark registry outbox, ranking evidence/calibration, replay export manifest, benchmark provenance, benchmark lifecycle, and ranker provenance mirror failures also avoid publishing local file-side metadata/object, ledger, settlement/outbox JSONL records, ranking JSONL evidence, replay-manifest, artifact/provenance, or staged service-local encrypted artifact objects for the failed operation. Review decisions in required mode preflight DB mirror availability before the trace-body read audit, mirror reviewed submission/object-ref/status state before file metadata, derived status, or file audit publication, and clean up a staged reviewed-envelope artifact if that required mirror fails. Revocation mirror failures in required mode preflight the DB tombstone/status/invalidation mirror plus the DB revocation-audit mirror before writing file-backed tombstones, status changes, provenance/benchmark invalidations, or file audit rows, so the cutover path does not publish file-only revocation state. Shared aggregate-audit and trace-content-read helpers also mirror the DB audit row before appending the compatibility file audit row in required mode, so analytics, reviewer reads, export source reads, and worker source reads cannot leave file-only audit rows when the mirror is missing or rejected. If a final export audit mirror fails after export metadata was staged, required mode removes the replay/benchmark/ranker file artifacts and deletes the DB export manifest/items/object refs for that export. Replay, benchmark, and ranker export DB mirrors write manifest metadata, export artifact object refs, and manifest item snapshots through one backend transaction so an item/reference validation failure does not leave a partial DB export.
...
pending ledger visible for hold/review or a higher governed cap. Operational
...
`TRACE_COMMONS_CREDIT_SETTLEMENT_ALLOWED_POLICY_VERSIONS` optionally pins a
comma-separated list of centrally approved settlement policy versions. Dry-runs
...
versions before writing settlement batches or NEAR outbox rows, and
source-list issuer approval recording rejects unlisted policy versions before
writing audit rows. Config status exposes only the configured allowlist count.
Operational summary and metrics also surface
`credit_settlement_policy_allowlist_missing` when positive delayed credit exists
without a configured allowlist, so centrally run issuance cannot look
rollout-ready before policy versions are pinned.
...
When this floor is enabled and `TRACE_COMMONS_RANKING_MIN_LABEL_SOURCE_COUNT` is
greater than `1`, model-risk and backtest reports also require the joined
pairwise labels to come from at least that many distinct label sources and
label-writing actor principals. Failures emit `pairwise_label_source_underdiverse`
or `pairwise_label_actor_underdiverse`, so one operator cannot satisfy pairwise
credit evidence merely by writing several source enums.
...
`TRACE_COMMONS_ARTIFACT_KEY_HEX` enables encrypted trace object storage. `TRACE_COMMONS_ENCRYPTED_ARTIFACTS=true` can be used as an explicit guard for the legacy encrypted artifact sidecar, but still requires the key. `TRACE_COMMONS_OBJECT_STORE=local_service` selects the production-shaped service-owned local backend and records DB object refs with the `trace_commons_service_local_encrypted` provider alias. That mode uses `TRACE_COMMONS_SERVICE_OBJECT_STORE_DIR` when set, otherwise `TRACE_COMMONS_ARTIFACT_DIR`, otherwise `TRACE_COMMONS_DATA_DIR/service_object_store`. `TRACE_COMMONS_OBJECT_STORE=remote_service` with `TRACE_COMMONS_REMOTE_OBJECT_STORE_PROVIDER=file_system` selects the filesystem-backed service-owned remote adapter, records DB object refs with the `trace_commons_service_owned_remote` provider alias, and uses an absolute `TRACE_COMMONS_REMOTE_OBJECT_STORE_BUCKET` path as the remote object root; AWS/GCS/Azure selections remain fail-closed behind `trace_commons_service_owned_remote_disabled`. Set `TRACE_COMMONS_REMOTE_OBJECT_STORE_FILE_SYSTEM_VERSIONING=true` on the filesystem-backed remote adapter to archive encrypted records on delete and allow restore-after-delete rehearsal across process restarts. In encrypted service-owned modes, submitted redacted envelopes, benchmark conversion artifacts, and ranker export provenance manifests are encrypted with TraceCommons secrets crypto, stored under tenant-hashed or tenant/submission-hashed artifact paths, and referenced by DB object refs. File-backed submission records retain envelope receipts so envelope reads resolve through encrypted object storage when present; benchmark/ranker export manifest items carry per-source object refs to the shared export artifact or provenance object. `TRACE_COMMONS_OBJECT_STORE_REQUIRE_VERSIONING=true` is a startup guard for cloud cutover candidates: it fails startup unless the selected service-owned object store reports both object versioning and restore-after-delete support. Local-service and default filesystem-remote adapters report those capabilities as false; versioned filesystem-remote can satisfy the guard for rollback drills, while AWS/GCS/Azure still require production provider adapters before cloud cutover. `TRACE_COMMONS_REMOTE_OBJECT_DELETER_URL` can be configured as a trusted deletion adapter for disabled AWS/GCS/Azure object refs during revocation propagation: requests include the exact object key needed by the adapter plus hash-only tenant/submission/key-ref evidence, and responses must return a canonical `sha256:` evidence hash before the server marks the DB object ref deleted. `TRACE_COMMONS_REMOTE_OBJECT_DELETER_BEARER_TOKEN` adds bearer auth and `TRACE_COMMONS_REMOTE_OBJECT_DELETER_TIMEOUT_MS` bounds calls. Set `TRACE_COMMONS_REMOTE_OBJECT_DELETER_REQUIRE_EXTERNAL=true` when production rollout requires the adapter even before a disabled cloud alias is active: startup fails unless the deleter URL is configured, config status exposes only configured/required/timeout fields, and operational summary plus metrics expose safe required/configured/ready booleans while blocking promotion on either `disabled_remote_object_deleter_missing` for disabled cloud stores or `remote_object_deleter_required_missing` for explicit production requirements. `TRACE_COMMONS_OBJECT_PRIMARY_SUBMIT_REVIEW=true` is a production-shaped submit/review cutover guard: it requires DB dual-write, required DB mirror writes, DB reviewer reads, reviewer object-ref reads, and an enabled service-owned object store (`local_service` or filesystem `remote_service`), then omits the plaintext submitted/reviewed envelope body files while still writing compatibility metadata, derived records, and file audit rows. `TRACE_COMMONS_OBJECT_PRIMARY_REPLAY_EXPORT=true` is the replay-export companion guard: it requires DB dual-write, required DB mirror writes, DB replay export reads, replay object-ref-required reads, and an enabled service-owned object store, then makes replay body exports use the existing DB object-ref path without file fallback. `TRACE_COMMONS_OBJECT_PRIMARY_DERIVED_EXPORTS=true` is the benchmark/ranker companion guard: it requires DB dual-write, required DB mirror writes, DB reviewer reads, required derived source object refs, export guardrails, and an enabled service-owned object store, then stores benchmark artifacts and ranker provenance only in encrypted object storage while keeping DB manifest/items as the durable index for purpose filters and lifecycle invalidation. The object-primary guards also accept `TRACE_COMMONS_OBJECT_PRIMARY_SUBMIT_REVIEW_TENANT_IDS`, `TRACE_COMMONS_OBJECT_PRIMARY_REPLAY_EXPORT_TENANT_IDS`, and `TRACE_COMMONS_OBJECT_PRIMARY_DERIVED_EXPORTS_TENANT_IDS` for tenant-by-tenant promotion behind the same dependency checks.
...
- `POST /v1/admin/ranking/readiness-drill`
...
- `POST /v1/admin/benchmark-readiness-drill`
...
and adapter-auth blocker counts for pending or submitted registry rows; the
benchmark readiness drill reports those same fields and folds them into its
evidence hash.
...
utility-attestation, ranking prediction-credit, process-evaluation utility
credit, benchmark conversion, and ranker export paths require a listed central
issuer principal before a credit ledger row is written. This keeps early
non-transferable credit issuance centralized while preserving non-positive
reviewer adjustments for fraud or abuse handling.
...
status shape as the workers and bounds failure-detail evidence before hashing:
submitted can recover pending/failed rows,
...
`TRACE_COMMONS_CREDIT_SETTLEMENT_ISSUER_APPROVAL_MAX_AGE_HOURS` can also
require that the recorded source-list approval is still fresh before live
...
external refs, or hold details. If review clears a held account, admins release
the specific hold through the release route before rerunning the risk summary or
settlement drill. The optional `limit` defaults to 100 accounts and is capped at
500; aggregate totals are computed before account-list truncation.
...
trace bodies. Export job worker routes require admin or export-worker
credentials and reject ordinary reviewer tokens before grant/job checks.
`POST /v1/workers/export/jobs/claim-and-run` supports queued
...
Set `TRACE_COMMONS_VECTOR_EMBEDDER_URL` to point at a trusted private embedding
adapter for vector-index payloads; `TRACE_COMMONS_VECTOR_EMBEDDER_BEARER_TOKEN`
adds bearer auth, `TRACE_COMMONS_VECTOR_EMBEDDER_TIMEOUT_MS` bounds calls, and
`TRACE_COMMONS_VECTOR_EMBEDDER_REQUIRE_EXTERNAL=true` makes vector indexing fail
closed when no adapter is configured. Embedder requests contain the redacted
canonical-summary embedding input, source/hash metadata, tool/category/coverage
tags, and a purpose hash; they omit raw trace bodies, contributor principals,
raw submission ids, and raw trace ids. If no adapter is configured and the
...
automation. Operational summary and Prometheus metrics split ranking worker-run
counts by automation kind and lifecycle status so process-evaluation health is
visible without raw trace details.
...
account enqueues `freeze_credit_account`, and release of the last active hold
...
rows. Hold placement and release audit rows mirror typed safe metadata with only
...
hashes and joined-evidence counts including distinct label actors, and a freshly
recomputed current-evidence snapshot with the effective server-owned thresholds,
error metrics, low-confidence prediction count, promotability flag, and reason
codes. Operators can compare these fields before activation without exposing
trace bodies or raw frontier-lab references. The promotion gate also requires
the current
...
deletion and receipt recording. When readable active vector payload artifacts
share the same vector store, model, version, dimension, and embedding algorithm,
the vector worker now uses their embedding values for nearest-neighbor
duplicate/novelty scoring and emits `embedding:` cluster ids; otherwise it falls
...
audits the vector-index pass. When a private vector embedder is configured, the
...
Retention workers should prefer the narrower `POST /v1/workers/retention-maintenance` route, which exposes only `purpose`, `dry_run`, `prune_export_cache`, `max_export_age_hours`, and `purge_expired_before`; the route accepts admin or retention-worker credentials and rejects ordinary reviewer tokens so reviewers cannot accidentally own purge automation. Revocation workers should use `POST /v1/workers/revocation-propagation`, which accepts admin or revocation-worker credentials, rejects ordinary reviewer tokens before DB mirror checks, requires the DB mirror, lists due tenant-scoped revocation-propagation ledger items, claims them with retry attempts, applies metadata/vector/export invalidation actions idempotently, appends deterministic negative credit-ledger reversal events for settled revoked sources, enqueues `reverse_credit_receipt` NEAR outbox calls for settled sources that had a NEAR contract, calls a configured worker-cache invalidator for exact hash-only process-evaluation and ranking-training queue targets, verifies and deletes service-owned encrypted submitted/rescrubbed/review envelope payloads for exact tenant/submission/object-ref targets, can call a configured remote object deleter for disabled cloud remote object refs, records a durable physical-delete receipt item with an evidence hash when deletion succeeds or was already recorded, marks unsupported physical-delete stores as skipped and keeps future artifact kinds out of deletion until explicitly mapped, gives disabled cloud remote object-store rows an explicit disabled-IO skip reason when no deleter is configured without exposing bucket/KMS/object-key details, audits dry-run or completed counts without reading trace bodies unless the action is a bounded object-payload delete, and projects the latest audited checked/completed/failed/skipped/pending counts plus disabled-remote deleter readiness into operational summary and Prometheus metrics. The revocation worker limit defaults to 100 and explicit limits outside 1..=500 are rejected.

Set `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_URL` to point revocation-propagation worker-queue items at a trusted invalidator adapter. The request is hash-only: it contains the tenant storage ref, revocation propagation item id, source submission ref hash, queue surface, and queue key hash, but no raw trace body, contributor token, tenant id, or submission id. The adapter must return a canonical `sha256:` `evidence_hash`, which is stored on the completed propagation item and used by the revocation-effects drill. `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_BEARER_TOKEN` adds bearer auth and `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_TIMEOUT_MS` bounds the call; HTTP endpoints are allowed only for localhost/loopback during development. When the adapter is absent, the worker preserves the existing local hash-only completion marker so pilots can rehearse ledger completion before deploying external cache operations. Set `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_REQUIRE_EXTERNAL=true` for production revocation propagation: startup fails unless the adapter URL is configured, worker-queue items fail closed instead of using the local completion marker when the adapter is missing, and operational summary plus metrics expose safe required/configured/ready booleans while blocking promotion on `worker_cache_invalidator_required_missing`. Config-status exposes only whether the adapter is configured, whether it is required, and the timeout.

Deployments can set `TRACE_COMMONS_RETENTION_MAINTENANCE_SCHEDULER_TOKEN` to run that same narrow retention-maintenance worker route in-process from a retention-worker token. Startup validates that the token can call the retention worker route, then the scheduler sleeps for `TRACE_COMMONS_RETENTION_MAINTENANCE_SCHEDULER_INTERVAL_SECONDS` (default 300) before applying the configured `TRACE_COMMONS_RETENTION_MAINTENANCE_SCHEDULER_DRY_RUN`, `TRACE_COMMONS_RETENTION_MAINTENANCE_SCHEDULER_PRUNE_EXPORT_CACHE`, `TRACE_COMMONS_RETENTION_MAINTENANCE_SCHEDULER_MAX_EXPORT_AGE_HOURS`, `TRACE_COMMONS_RETENTION_MAINTENANCE_SCHEDULER_PURGE_EXPIRED_BEFORE`, and `TRACE_COMMONS_RETENTION_MAINTENANCE_SCHEDULER_PURPOSE` values. Config status exposes only scheduler presence, interval, dry-run status, prune settings, max-age, and whether a purge cutoff is configured; it never returns the worker token, raw purpose, or purge timestamp.

Deployments can set `TRACE_COMMONS_REVOCATION_PROPAGATION_SCHEDULER_TOKEN` to run that same bounded revocation-propagation worker route in-process on an interval from a revocation-worker token. The scheduler requires the DB mirror at startup, supports bounded `TRACE_COMMONS_REVOCATION_PROPAGATION_SCHEDULER_INTERVAL_SECONDS`, `TRACE_COMMONS_REVOCATION_PROPAGATION_SCHEDULER_LIMIT`, `TRACE_COMMONS_REVOCATION_PROPAGATION_SCHEDULER_DRY_RUN`, and `TRACE_COMMONS_REVOCATION_PROPAGATION_SCHEDULER_PURPOSE`, and config-status exposes only scheduler presence, interval, limit, and dry-run state.

Set `purge_expired_before` to an explicit RFC3339 cutoff plus a non-empty `purpose` to mark already-expired submissions purged and delete their file-backed plaintext envelope bodies plus service-owned encrypted local or filesystem-remote submitted-envelope artifacts; dry-run purge previews may omit purpose and must not delete those payloads. When a non-dry-run purge or revocation-propagation object-payload delete physically removes a service-owned encrypted trace body, the DB mirror marks only the matching tenant/submission/object-store/object-key object ref with `deleted_at`, while expiration and revocation continue to invalidate refs without claiming physical deletion. Revocation-propagation object deletes also upsert a tenant-scoped `PhysicalDeleteReceipt` propagation row so retries can backfill the receipt after a successful delete and reconciliation can distinguish invalidation from proven payload removal.
...
still cannot. `TRACE_COMMONS_REMOTE_OBJECT_STORE_FILE_SYSTEM_VERSIONING=true`
enables encrypted archive-on-delete plus restore rehearsal for the filesystem
remote adapter. `TRACE_COMMONS_OBJECT_STORE_REQUIRE_VERSIONING=true` is the
separate cloud-cutover guard: startup fails unless the configured store also
reports object versioning and restore-after-delete support, so local-service and
default filesystem-remote fail closed while versioned filesystem-remote can run
rollback drills before a cloud provider is compiled. The server-owned
`V1__trace_commons_schema.sql` through
`V18__trace_central_rls_tenant_predicate.sql` migrations cover the
...
hashes, settlement calibration evidence-hash binding, and the central
`trace_current_tenant_id()` RLS policy predicate; config-status can report
catalog-only RLS readiness, including policy counts, central-predicate
expression mismatches, disabled tables, force-RLS counts, force-RLS missing
tables, the current runtime role hash, whether the current role bypasses RLS,
whether the current role owns Trace Commons tables, whether an expected runtime
role hash is configured, whether it matches, and stricter production-ready
booleans. The admin operational summary and metrics promote unsafe RLS
diagnostics into `postgres_trace_rls_not_ready` and
`postgres_trace_runtime_role_hash_mismatch` promotion blockers while exposing
only readiness booleans and aggregate counts, not table names.
...
as an active tenant boundary, and can set
`TRACE_COMMONS_POSTGRES_RUNTIME_ROLE_SHA256` to make startup, the RLS drill, and
promotion gates prove that the deployed connection is the intended service role.
The Rust PostgreSQL backend now keeps raw pool access out of the normal public
application API, with crate-internal store access and an explicitly named
test/diagnostic hook for raw RLS probes. Production deployments still need to
run the full smoke suite under that pinned non-bypassing non-owner role before
RLS can become the active trust boundary. Production RLS tests already exercise
...
- File-backed compatibility reads now also fail closed if a metadata or derived row stored under one tenant directory claims a different embedded tenant id or tenant storage ref. File-backed control-plane writes reject the same embedded tenant drift before persisting tenant-local submission metadata, derived metadata, revocation tombstones, credit, audit, replay, benchmark, ranking, settlement, hold, outbox, and provenance rows. Continue moving mutating paths to `TenantCtx` so authenticated tenant identity stays the trust boundary after rehydration.
...
- Promote deterministic novelty/duplicate scoring into private vector-backed feature workers. The vector worker can now use a private embedder, compatible stored vector payloads, and an optional private vector-search adapter whose candidate ids are revalidated against active tenant DB vector metadata before they affect nearest-neighbor, cluster, duplicate, or novelty fields. Operational summary, metrics, and `/v1/admin/vector-index-drill` now flag active vector entries whose stored nearest-neighbor refs cannot be resolved to an active compatible vector profile, so cross-model or stale duplicate/novelty evidence blocks promotion. The first ranking feature worker already reserves `feature_provenance:server_derived`, can require active vector metadata for vector-backed duplicate/novelty inputs, and can be required for prediction-credit, readiness, and settlement with `TRACE_COMMONS_RANKING_REQUIRE_SERVER_FEATURE_PROVENANCE=true`; production still needs deployed vector-store operations and rehearsed canary evidence.

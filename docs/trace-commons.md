...
artifact instead of DB rows. Deployments can use
...
| Vector duplicate/novelty index | Partial | DB schema, storage contract, dedicated vector worker route, maintenance-triggered metadata indexer, object-ref/readability gating before non-dry-run vector writes, per-source vector-index content-read audits with source object refs, admin-safe vector-entry metadata listing, and reconciliation gap diagnostics now persist/vector-check vector-entry metadata, nearest trace ids, cluster id, duplicate score, novelty score, and invalidation state for accepted canonical summaries. Duplicate/novelty scoring still uses deterministic redacted-summary similarity with exact-hash matches as the strongest signal, but the vector worker can now call a trusted private embedding adapter for payload embeddings, persist adapter model/version/dimension/vector-store metadata, and fail closed when external embeddings are required. Vector payload search and model-backed duplicate/novelty workers are still future work. |

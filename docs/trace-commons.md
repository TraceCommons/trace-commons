...
The worker route is intentionally limited to `regression_catch`, `training_utility`, and `ranking_utility`; it is not for `reviewer_bonus` or `abuse_penalty`. Ranking utility events are credit-bearing only when the external reference is a single `ranking_prediction:<uuid>` for the same source.
...
be `promotable`. Each selected ranking utility credit event must also reference
...
the calibration run id and report hash used for the gate.

...
bounded ranking calibration, prediction-credit, and model-promotion automation. Rows include
...
prediction-credit, and model-promotion runs refuse active overlapping non-stale rows before appending a
...
`TRACE_COMMONS_RANKING_MIN_LABEL_SOURCE_COUNT`, and per-source cohort error
gates on top so the sample is high-quality and broad enough across
reviewers/labs. Direct registration of an `active` model uses the same
calibration freshness and diversity gates as the explicit model-promotion route.
...
outbox submit workers in that order. `submit_near_outbox` defaults to false, so
the final step inspects pending NEAR payloads as a dry-run unless the operator
explicitly asks the trusted relayer submission step to run live.

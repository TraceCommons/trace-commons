...
- Ranking production policy includes `TRACE_COMMONS_RANKING_REQUIRE_CALIBRATION_DATASET_REGISTRY`, `TRACE_COMMONS_RANKING_REQUIRE_ACTIVE_CALIBRATION_DATASET`, `TRACE_COMMONS_RANKING_REQUIRE_SERVER_FEATURE_PROVENANCE`, `TRACE_COMMONS_RANKING_MIN_LABEL_COUNT`, `TRACE_COMMONS_RANKING_MIN_LABEL_SOURCE_COUNT`, `TRACE_COMMONS_RANKING_MIN_CONFIDENCE_THRESHOLD`, `TRACE_COMMONS_RANKING_MAX_AVERAGE_ABSOLUTE_ERROR_MICROS`, `TRACE_COMMONS_RANKING_MIN_PAIRWISE_LABEL_COUNT`, and `TRACE_COMMONS_RANKING_MIN_PAIRWISE_ACCURACY_MICROS`, server-owned quality gates applied after worker request parsing and model-risk recomputation so calibration and prediction-credit workers cannot bypass active holdout stewardship, holdout registration, server-derived feature provenance, or the deployment's minimum sample-size, source/actor diversity, ordering-accuracy, or quality requirements. When a pairwise evidence floor is enabled and the label-source floor is greater than one, pairwise checks reuse that diversity floor for distinct preference-label sources and distinct label-writing actor principals; stored calibration gates also re-check distinct label-writing actor principals so legacy source-diverse runs by one actor emit `calibration_label_actor_underdiverse`. Startup rejects active holdout stewardship unless the calibration dataset registry gate is also enabled, and rejects an explicitly configured nonzero pairwise ordering-accuracy floor unless the pairwise evidence floor is greater than zero.

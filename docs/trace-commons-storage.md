...
- `migrations/V21__trace_near_credit_account_outbox.sql`, adding a separate NEAR account-operation outbox for credit-hold freeze/unfreeze calls without weakening settlement receipt constraints.
...
- `migrations/V20__trace_credit_settlement_issuer_approval_hash.sql`, binding finalized credit settlement batches to optional central issuer approval evidence.
...
- `/v1/admin/operational-summary` and `/v1/admin/operational-metrics` expose safe NEAR non-transferable credit outbox readiness: aggregate `pending`, `submitted`, `confirmed`, `failed`, `pending_without_submitter`, and `submitted_without_confirmer` counts plus submitter/confirmer configured flags. Those projections include settlement receipt rows and credit-hold account freeze/unfreeze rows, making pending rows without a submitter, submitted rows without a confirmer, and failed outbox rows promotion blockers without exposing transaction hashes, account hashes, contract ids, adapter URLs, raw failure text, or oversized failure details.
- Credit-settlement readiness in the same summary/metrics also exposes only booleans for the configured-contract requirement, configured-contract presence, per-account cap, policy-version allowlist configuration and missing-allowlist blocker state for positive delayed credit, submitter/confirmer adapters, central issuer approval gate, central issuer profile readiness, managed EdDSA enforcement, and tenant access-grant enforcement, plus aggregate held-account, over-cap account, and credit-risk truncation blockers and a safe missing-control count for the central issuer profile. Config status additionally exposes whether live settlement is configured to require rollout-smoke readiness and how many hashed central issuer principal refs are pinned.
- The credit-settlement drill response includes `credit_settlement_central_issuer_profile_missing_controls`, the same safe env-var names returned by config status, so a failed central issuer profile rehearsal can be fixed without exposing adapter URLs, bearer tokens, approval evidence, account refs, or raw issuer principals.
- The credit-settlement drill accepts the same bounded `source_event_limit` shape as admin, worker, and scheduler settlement runs. Operators can therefore rehearse a partial batch, review the exact returned `source_list_hash`, record a hash-only issuer approval for that batch, and then finalize the matching admin/worker/scheduler run without approving the whole backlog.
- Live credit settlement re-checks that central issuer profile before writing settlement batches or NEAR outbox rows; when `TRACE_COMMONS_CREDIT_SETTLEMENT_CENTRAL_ISSUER_PRINCIPAL_REFS` is configured, only listed hashed principals can record issuer approvals, create or release credit holds, finalize live settlement, run live settlement/credit-cycle schedulers, run live NEAR credit outbox submit/confirm workers or schedulers, manually mark NEAR outbox status, create credit-hold NEAR account freeze/unfreeze transitions, or create positive credit through manual delayed-credit mutation, utility-credit jobs, utility attestations, ranking prediction-credit, process-evaluation utility credit, or credit-bearing benchmark/ranker export generation. Positive-credit preflights run before source reads, process-evaluation envelope rewrites, benchmark artifacts, ranker provenance, worker-run ledgers, or queued credit-bearing export-job claims are created. When `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_ROLLOUT_SMOKE_READY=true`, live settlement also requires fresh green rollout-smoke readiness before any live repair/write path. Dry-runs, drills, credit-cycle preflights, and NEAR outbox dry-runs remain diagnostic surfaces instead of mutating fallbacks.
- Live credit-cycle worker runs also check the central issuer profile before claiming work, and scheduler preflight reports `credit_settlement_central_issuer_profile_incomplete` or `credit_settlement_rollout_smoke_not_ready` as skip reasons rather than starting partial cycle side effects. Live in-process scheduler startup is rejected, and live route-triggered scheduler runs skip with `credit_settlement_source_list_approval_required` before worker claim, when exact source-list issuer approval is required; dry-run/preflight remains available for discovery before separately approved settlement.
- Admin-only `GET|POST /v1/admin/credit-settlement-approvals` records and lists hash-only central issuer approvals. Each approval is bound to a tenant, policy version, canonical dry-run settlement `source_list_hash`, and canonical lowercase `sha256:<64 hex>` evidence hash; raw approval reasons and external references are stored only as hashes in the audit chain. When `TRACE_COMMONS_CREDIT_SETTLEMENT_ALLOWED_POLICY_VERSIONS` is configured, approval recording rejects unlisted policy versions before audit persistence. When `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_ISSUER_APPROVAL=true`, live settlement requires the supplied approval hash to match a recorded approval for the exact source list before settlement batches or NEAR outbox rows are written.
- Across credit issuance, NEAR receipt payloads, rollout-smoke evidence, and ranking evidence, canonical `sha256:<64 hex>` means a lowercase hex digest; uppercase hex variants are rejected before persistence or outbox submission.
...
- Ranking production policy includes `TRACE_COMMONS_RANKING_REQUIRE_CALIBRATION_DATASET_REGISTRY`, `TRACE_COMMONS_RANKING_REQUIRE_ACTIVE_CALIBRATION_DATASET`, `TRACE_COMMONS_RANKING_REQUIRE_SERVER_FEATURE_PROVENANCE`, `TRACE_COMMONS_RANKING_MIN_LABEL_COUNT`, `TRACE_COMMONS_RANKING_MIN_LABEL_SOURCE_COUNT`, `TRACE_COMMONS_RANKING_MIN_CONFIDENCE_THRESHOLD`, `TRACE_COMMONS_RANKING_MAX_AVERAGE_ABSOLUTE_ERROR_MICROS`, `TRACE_COMMONS_RANKING_MIN_PAIRWISE_LABEL_COUNT`, `TRACE_COMMONS_RANKING_MIN_PAIRWISE_ACCURACY_MICROS`, optional `TRACE_COMMONS_RANKING_MAX_LABELER_ISSUE_RATE_MICROS`, and optional `TRACE_COMMONS_RANKING_MIN_LABELER_RELIABILITY_LABEL_COUNT`, server-owned quality gates applied after worker request parsing and model-risk recomputation so calibration and prediction-credit workers cannot bypass active holdout stewardship, holdout registration, server-derived feature provenance, or the deployment's minimum sample-size, source/actor diversity, ordering-accuracy, labeler-reliability, or quality requirements. When a pairwise evidence floor is enabled and the label-source floor is greater than one, pairwise checks reuse that diversity floor for distinct preference-label sources and distinct label-writing actor principals; stored calibration gates also re-check distinct label-writing actor principals so legacy source-diverse runs by one actor emit `calibration_label_actor_underdiverse`. When a labeler reliability ceiling or floor is configured, active model-risk, prediction-credit issuance, credit-readiness, and settlement recompute the current calibration label sources plus hashed actors and block with `calibration_label_source_issue_rate_above_threshold`, `calibration_label_actor_issue_rate_above_threshold`, `calibration_label_source_reliability_support_below_threshold`, or `calibration_label_actor_reliability_support_below_threshold` if their tenant-wide reliability row is outside policy. Startup rejects active holdout stewardship unless the calibration dataset registry gate is also enabled, and rejects an explicitly configured nonzero pairwise ordering-accuracy floor unless the pairwise evidence floor is greater than zero.
...
- Model-promotion dry-runs now act as the candidate holdout-readiness preflight: responses include the registered calibration dataset hash, stored calibration report/joined-evidence hashes and counts, distinct joined label-actor counts, freshly recomputed current evidence hash/report hash/counts, effective server-owned thresholds, aggregate/per-source error metrics, low-confidence prediction count, promotability flag, and reason codes without exposing trace bodies or raw lab references. Promotion also requires the current model backtest to pass, so pairwise evidence, source/actor diversity, ordering failures, or unresolved label-adjudication issues block activation before a candidate can mint or settle ranking credit.
- Admin ranking model-backtest inspection through `/v1/admin/ranking/model-backtest-report`, which applies current calibration, pairwise preference, and label-adjudication checks to latest candidate and active model/target-use pairs, returns pass/fail counts, latest calibration hashes, joined-evidence/error metrics, pairwise source/actor diversity, pairwise accuracy, and the same machine-readable reason codes before promotion or credit issuance.
- Admin ranking model-risk inspection through `/v1/admin/ranking/model-risk-report`, which recomputes current joined-evidence hashes for active model/target-use pairs and reports current joined-label source/actor diversity, calibration thresholds, aggregate/per-source error metrics, pairwise preference source/actor diversity and ordering accuracy, configured calibration-labeler reliability blockers, post-calibration evidence drift, low-confidence fresh predictions, stale/non-promotable calibration status, training/calibration dataset overlap, aggregate risk-code counts, and per-model risk codes without exposing trace bodies or raw lab references.
- Admin ranking credit readiness inspection through `/v1/admin/ranking/credit-readiness-report`, which explains whether pending positive ranking utility credit events are ready to settle or blocked by prediction refs, model status, calibration freshness/promotability/diversity, configured calibration-labeler reliability blockers, held accounts, score mismatches, confidence gates, calibration-registry manifest conflicts, or uncleared active-model risk codes.
- Ranking prediction-credit issuance and ranking-utility readiness/settlement all enforce the latest calibration confidence threshold, configured calibration-labeler reliability floors/ceilings, and active-model risk report for the referenced active-model prediction, so low-confidence, unreliable-labeler, cold-start-labeler, or currently at-risk predictions can be monitored but cannot mint or settle credit.
...
- `TRACE_COMMONS_OBJECT_STORE=remote_service` now parses provider, bucket/root, KMS key, and service credential references, rejecting missing, oversized, or control-character-bearing values without echoing raw config. `TRACE_COMMONS_REMOTE_OBJECT_STORE_PROVIDER=file_system` enables a filesystem-backed service-owned remote adapter for production-like object I/O rehearsals, using `TRACE_COMMONS_REMOTE_OBJECT_STORE_BUCKET` as an absolute root path plus `TRACE_COMMONS_ARTIFACT_KEY_HEX` for envelope encryption. Filesystem-remote writes validate encrypted object records before persistence, reject same-key divergent overwrites, and commit JSON records through synced same-directory temp files plus rename so object paths are replaced atomically instead of following pre-existing symlinks. Submission metadata now records the encrypted artifact object-store alias when an artifact receipt is stored; DB object-ref backfill preserves that recorded alias across later object-store config changes, and encrypted object-ref reads fail closed when the configured store does not match the recorded alias. Reconciliation reports these as `store_mismatched_active_envelope_object_refs`, separate from unreadable, hash-mismatched, or key-ref-mismatched objects, and the DB reconciliation drill returns explicit safe counts for each active-envelope object-ref failure class. `TRACE_COMMONS_REMOTE_OBJECT_STORE_FILE_SYSTEM_VERSIONING=true` makes filesystem-remote deletes archive encrypted object records under the service-owned root so rollback drills can restore by object ref across process restarts. AWS/GCS/Azure provider selections still advertise the disabled `trace_commons_service_owned_remote_disabled` alias, refuse artifact I/O and plaintext fallback, and do not satisfy object-primary or required-versioning startup guards until those adapters are compiled.
...
- Durable DB revocation-propagation item rows track tenant-scoped downstream invalidation or retry work for object refs, export manifests/items, vectors, derived artifacts, benchmark/ranker artifacts, operator-raised credit settlement reversals, and physical delete receipts.
- Replay dataset, benchmark conversion, and ranker export paths persist durable access-grant and export-job lifecycle rows; already-started jobs are terminalized as `failed` if DB metadata reads, retention metadata validation, source collection, source object-ref revalidation, source-read audit mirroring, or required object-ref body reads fail before export artifacts or manifests can be published. `TRACE_COMMONS_EXPORT_JOB_SCHEDULER_TOKEN` enables an optional in-process scheduler that authenticates with an export-worker bearer token, retries due failed replayable jobs with bounded exponential backoff, then drains queued jobs through the same worker handlers used by external schedulers. `TRACE_COMMONS_RETENTION_MAINTENANCE_SCHEDULER_TOKEN` enables an optional in-process scheduler that authenticates with a retention-worker bearer token and runs the same narrow `/v1/workers/retention-maintenance` route on an interval for expiration, purge, export-cache pruning, retention-ledger, and provenance invalidation work; config status reports only scheduler shape and never the token, purpose, or purge cutoff. `TRACE_COMMONS_VECTOR_INDEX_SCHEDULER_TOKEN` enables an optional in-process scheduler that authenticates with a vector-worker bearer token, requires the DB mirror at startup, and runs the same bounded `/v1/workers/vector-index` route on an interval without retention-maintenance side effects. `TRACE_COMMONS_REVOCATION_PROPAGATION_SCHEDULER_TOKEN` enables an optional in-process scheduler that authenticates with a revocation-worker bearer token, requires the DB mirror at startup, and runs the same bounded revocation-propagation worker route; dry-run mode audits due counts while leaving propagation rows unclaimed. `TRACE_COMMONS_PROCESS_EVALUATION_SCHEDULER_TOKEN` enables an optional in-process scheduler that authenticates with a process-evaluation worker token, fails closed by default unless an external process evaluator adapter is configured, and runs the same bounded process-evaluation worker route; dry-run mode records only the safe worker-run ledger without evaluator calls, trace writes, or ranking-label writes. `TRACE_COMMONS_BENCHMARK_PIPELINE_SCHEDULER_TOKEN` enables an optional in-process scheduler that authenticates with a benchmark-worker bearer token, fails closed by default unless an external evaluator adapter is configured, then runs bounded benchmark evaluation followed by bounded local registry publication through the same worker handlers; dry-run mode performs no evaluator calls, lifecycle writes, or outbox writes. `TRACE_COMMONS_BENCHMARK_REGISTRY_SCHEDULER_TOKEN` enables an optional in-process scheduler that authenticates with a benchmark-worker bearer token, submit-drains pending/failed benchmark registry outbox rows, then confirms submitted rows through the same bounded worker routes; live mode requires configured submitter and confirmation adapters at startup, while dry-run mode can rehearse counts and audit rows without those adapters.
- `TRACE_COMMONS_BENCHMARK_REGISTRY_REQUIRE_ADAPTER_AUTH=true` adds a production
  fail-closed gate for live benchmark-registry submit, confirm, scheduler, and
  manual submitted/confirmed fallback paths. Config status exposes only safe
  submitter/confirmer auth-readiness booleans plus the require-auth toggle, not
  adapter URLs or bearer tokens. Operational summary and metrics also turn
  pending/submitted registry rows into explicit adapter-auth blockers when the
  adapter is present but the required bearer credential is absent. The benchmark
  readiness drill exposes the same safe auth-readiness fields and blocker counts
  in its response and evidence hash.
...
7. Enable production retention and revocation propagation.
   - Revocation writes tombstones first, then invalidates submissions, derived rows, vectors, benchmarks, exports, worker queues, and settled credit.
   - Configure `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_URL` and `TRACE_COMMONS_WORKER_CACHE_INVALIDATOR_REQUIRE_EXTERNAL=true` before live worker-queue revocation propagation so missing external cache invalidation fails closed and appears as `worker_cache_invalidator_required_missing` in operational promotion gates.
...
- Run `POST /v1/admin/postgres-rls-drill` with `record_evidence: true` before enabling DB reader promotion. A green response has `ready: true`, complete policy/RLS/FORCE RLS coverage, transaction-local tenant context, a non-bypassing non-owner runtime role, a matching expected runtime role hash when `TRACE_COMMONS_POSTGRES_RUNTIME_ROLE_SHA256` is configured, no `blocking_gaps`, and a recorded `postgres_rls_readiness` evidence row. A service without a DB mirror returns a `TRACE_COMMONS_DB_DUAL_WRITE` operator error instead of pretending RLS evidence exists.
- Treat missing policies, disabled RLS, disabled FORCE RLS, policy-expression drift, sticky tenant context, a bypassing runtime role, a runtime role that owns Trace Commons tables, or `runtime_role_hash_mismatch` as blockers, even if ordinary DB reads appear to work during a canary.
...
- `TRACE_COMMONS_OBJECT_STORE=remote_service` with `TRACE_COMMONS_REMOTE_OBJECT_STORE_PROVIDER=file_system` enables the service-owned filesystem-backed remote adapter. Use an absolute `TRACE_COMMONS_REMOTE_OBJECT_STORE_BUCKET` path as the remote object root, configure KMS and credential references for operator intent, and provide `TRACE_COMMONS_ARTIFACT_KEY_HEX` for ciphertext envelope encryption. Remote provider, bucket/root, KMS, and credential refs must be single-line bounded values; parser and status errors name the config field but do not echo the raw value. Set `TRACE_COMMONS_REMOTE_OBJECT_STORE_FILE_SYSTEM_VERSIONING=true` to retain encrypted deleted versions for restore rehearsal. AWS/GCS/Azure selections still fail closed behind `trace_commons_service_owned_remote_disabled`; if that disabled alias is selected, `TRACE_COMMONS_REMOTE_OBJECT_DELETER_URL` must also be configured before operational promotion can go green. Production rollouts can set `TRACE_COMMONS_REMOTE_OBJECT_DELETER_REQUIRE_EXTERNAL=true` to require that adapter independently of the current object-store alias. Summary/metrics expose only safe deleter required/configured/ready booleans, with `disabled_remote_object_deleter_missing` for disabled cloud stores and `remote_object_deleter_required_missing` for explicit production requirements.
...
- DB reconciliation: `/v1/admin/db-reconciliation-drill` returns `ready: true`, records fresh `db_reconciliation_clean` evidence, and has empty `blocking_gaps`, including current retention job/item gaps, credit-ledger gaps, credit settlement control-plane gaps, audit-event gaps, ranking calibration dataset manifest-conflict keys, reader-projection parity gaps, and object-ref readability/hash/key-ref/store-alias gaps. The response also exposes aggregate active-envelope object-ref failure counts, not raw object keys.
...
- Revocation propagation: when external worker-cache invalidation is required, operational summary and `trace_commons_operational_revocation_propagation_readiness` metrics must show required/configured/ready as true before live runs, and a missing adapter must block promotion without exposing raw tenant, submission, or queue ids.
...
- Credit settlement: set `TRACE_COMMONS_CREDIT_SETTLEMENT_MAX_POINTS_PER_ACCOUNT`, `TRACE_COMMONS_CREDIT_SETTLEMENT_ALLOWED_POLICY_VERSIONS`, `TRACE_COMMONS_CREDIT_SETTLEMENT_NEAR_CONTRACT_ID`, `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_NEAR_CONTRACT=true`, `TRACE_COMMONS_NEAR_CREDIT_SUBMITTER_URL`, and `TRACE_COMMONS_NEAR_CREDIT_CONFIRMATION_URL` for production issuers; set `TRACE_COMMONS_NEAR_CREDIT_REQUIRE_ADAPTER_AUTH=true` with both NEAR bearer-token env vars when the relayer and confirmer are no longer pilot-private. Early production deployments that centrally run issuance can set `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_CENTRAL_ISSUER_PROFILE=true` to fail startup unless DB mirror writes, PostgreSQL RLS readiness, managed EdDSA signed-token enforcement plus tenant access-grant enforcement, exact hashed issuer principals in `TRACE_COMMONS_CREDIT_SETTLEMENT_CENTRAL_ISSUER_PRINCIPAL_REFS`, per-account caps, approved settlement policy versions, fresh source-list issuer approvals, pinned NEAR contract settlement, submit/confirm adapters, adapter bearer auth, and `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_ROLLOUT_SMOKE_READY=true` are all enabled, avoiding a premature actor trust model while keeping live issuance controls explicit. When that principal allowlist is configured, unlisted admins can inspect dry-runs and hold lists but cannot record source-list approvals, create or release credit holds, or finalize live settlement, and unlisted utility-worker scheduler tokens fail live startup validation unless the scheduler is dry-run or credit-cycle preflight-only. Then run `POST /v1/admin/credit-settlement-drill` with `record_evidence: true`, the production policy version, and the non-transferable NEAR contract id. A green response has `ready: true`, `policy_version_allowed: true`, `near_contract_configured: true`, `near_submitter_configured: true`, `near_confirmer_configured: true`, `near_submitter_auth_configured: true` and `near_confirmer_auth_configured: true` when adapter auth is required, `settlement_account_cap_configured: true`, `risk_summary.truncated: false`, no held or over-cap accounts, no settlement/ranking exclusion reason counts, nonzero pending dry-run settlement work unless `require_pending` is explicitly disabled, and a recorded `credit_settlement` evidence row without writing settlement batches or NEAR outbox rows. When `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_ISSUER_APPROVAL=true`, first run the drill or settlement dry-run to capture the exact canonical `source_list_hash`, record the central hash-only approval with `POST /v1/admin/credit-settlement-approvals` using the production `policy_version`, that `source_list_hash`, and a canonical `sha256:<64 hex>` approval `evidence_hash`, then include the same hash in the drill and live settlement requests; if a policy allowlist is configured, that approval route also rejects unlisted policy versions before writing audit rows. To let the server run utility-worker settlement itself, configure `TRACE_COMMONS_CREDIT_SETTLEMENT_SCHEDULER_TOKEN`, `TRACE_COMMONS_CREDIT_SETTLEMENT_SCHEDULER_POLICY_VERSION`, an optional approval hash/NEAR contract/ranking gate, and `TRACE_COMMONS_CREDIT_SETTLEMENT_SCHEDULER_DRY_RUN=true` until the rehearsal is green; the live scheduler token must also appear in the central issuer principal allowlist when that allowlist is configured. Operational summary should show `credit_settlement_scheduler_enabled` and metrics should expose `scheduler_enabled=1` without raw scheduler config. The drill records `credit_settlement_policy_version_not_allowed` until the requested policy version is listed, `issuer_approval_evidence_hash_missing` until a hash is supplied, `issuer_approval_evidence_hash_unrecorded` until that hash is recorded for the source list, and live settlement fails closed before writing batches or outbox rows unless the policy version is allowed, the recorded approval matches, and rollout-smoke readiness is green when required. For failure rehearsal, verify positive delayed credit without an allowlist produces the operational `credit_settlement_policy_allowlist_missing` blocker, an issuer approval for an unlisted policy fails without an audit row, a missing cap produces `settlement_account_cap_missing`, missing central issuer profile config produces `credit_settlement_central_issuer_profile_incomplete`, an unapproved policy produces `credit_settlement_policy_version_not_allowed`, missing configured NEAR contract produces `near_contract_id_missing` or `credit_settlement_near_contract_missing`, missing NEAR adapters produce `near_credit_submitter_missing` or `near_credit_confirmer_missing`, missing adapter bearer tokens produce `near_credit_submitter_auth_missing` or `near_credit_confirmer_auth_missing` when auth is required, disabled managed-EdDSA, missing tenant access-grant enforcement, missing central issuer principal refs, or unlisted live scheduler principals keep the central issuer path from writing live credit, missing or unrecorded issuer approval produces the corresponding issuer blocker, rollout-smoke gaps block live settlement with safe evidence-status/count fields, and held or over-cap accounts produce blocker codes such as `held_credit_accounts_present`, `over_cap_credit_accounts_present`, and `account_settlement_amount_exceeds_cap`, then release the test hold through `/v1/admin/credit-holds/{hold_id}/release` and rerun the drill to prove settlement resumes only after the risk projection is clean.
- Central issuer profile mode also requires `TRACE_COMMONS_POSTGRES_RUNTIME_ROLE_SHA256`; a missing expected PostgreSQL runtime-role hash keeps `credit_settlement_central_issuer_profile_ready` false, and a mismatched hash appears as `runtime_role_hash_mismatch` in the PostgreSQL RLS drill plus `postgres_trace_runtime_role_hash_mismatch` in operational promotion gates.
- When issuer approval is required for a bounded live settlement, run `POST /v1/admin/credit-settlement-drill` with the same `source_event_limit` as the admin settlement request, worker, or settlement scheduler, record the returned `source_list_hash` through `POST /v1/admin/credit-settlement-approvals`, and use the same approval hash on the live admin/worker/scheduler request. Omit `source_event_limit` only when the approval is meant to cover the whole current backlog.
- During that failure rehearsal, inspect `credit_settlement_central_issuer_profile_missing_controls` in the drill response and `central_issuer_profile_missing_control_count`, `policy_allowlist_missing`, `managed_eddsa_signed_tokens_required`, and `tenant_access_grants_required` in `trace_commons_operational_credit_settlement_readiness` metrics before turning on live settlement.
- Confirm an incomplete central issuer profile rejects live settlement with `live credit settlement requires complete central issuer profile` while leaving dry-run settlement available and writing no settlement batches or NEAR outbox rows.
...
- Ranking credit: set `TRACE_COMMONS_RANKING_CALIBRATION_MAX_AGE_HOURS`, `TRACE_COMMONS_RANKING_MIN_LABEL_COUNT`, `TRACE_COMMONS_RANKING_MIN_CONFIDENCE_THRESHOLD`, `TRACE_COMMONS_RANKING_MAX_AVERAGE_ABSOLUTE_ERROR_MICROS`, `TRACE_COMMONS_RANKING_MIN_LABEL_SOURCE_COUNT`, `TRACE_COMMONS_RANKING_MIN_PAIRWISE_LABEL_COUNT`, `TRACE_COMMONS_RANKING_MIN_PAIRWISE_ACCURACY_MICROS`, and optionally `TRACE_COMMONS_RANKING_MAX_LABELER_ISSUE_RATE_MICROS` plus `TRACE_COMMONS_RANKING_MIN_LABELER_RELIABILITY_LABEL_COUNT` for production issuers; the pairwise accuracy floor must be paired with a nonzero pairwise label-count floor, and pairwise labels must clear the configured label-source and actor-principal diversity floor when that floor is greater than one. Then verify stale calibration, insufficient joined-label sample size, duplicate same-source labels that should not count as independent evidence, multiple label-source enums written by the same actor principal, joined-evidence drift after calibration, low-confidence predictions, excessive aggregate/per-source error, insufficient label-source diversity, insufficient calibration actor diversity, over-threshold calibration labeler issue rates, insufficient calibration labeler reliability support, insufficient pairwise evidence, insufficient pairwise source/actor diversity, weak pairwise ordering accuracy, training/calibration dataset overlap, calibration-registry manifest conflicts, and a bad per-source calibration cohort block model promotion, prediction-credit issuance, and ranking-utility settlement before NEAR outbox submission or confirmation is enabled.
- Revocation propagation: run `POST /v1/admin/revocation-propagation-drill` with `record_evidence: true` after creating a canary revocation. A green response has `ready: true`, `dry_run: true`, no `blocking_gaps`, pending due-item counts, and a recorded `revocation_propagation` evidence row without claiming items. After the live worker runs, run `POST /v1/admin/revocation-effects-drill` with the canary `submission_id` and `record_evidence: true`; a green response records `delayed_credit_reversal`, `object_deletion_refs`, and `worker_queue_invalidation` evidence by proving that no settled-credit reversal item or `reverse_credit_receipt` NEAR outbox row was raised by the revocation, that any operator-raised reversal item is fully drained, and by proving deleted service-owned object refs, physical-delete receipts, hash-only invalidation rows for process-evaluation and ranking-training worker surfaces, and filesystem-remote artifact removal without exposing raw trace bodies or object keys.
- Object-primary: after submitting an accepted canary trace with a service-owned object store, run `POST /v1/admin/object-primary-read-drill` with the canary `submission_id`, a fallback `fallback_tenant_id`, and `record_evidence: true`. A green response records fresh `object_primary_reads` evidence, verifies tenant key refs and hashes for submitted/review/replay reads, proves the submitted plaintext body file is absent, and confirms the fallback tenant remains outside object-primary rollout. Disabled cloud `remote_service` providers stay blocked until configured, and future artifact kinds must be explicitly mapped rather than silently succeeding.
...
- After parity checks pass, promote critical writes with `TRACE_COMMONS_REQUIRE_DB_MIRROR_WRITES=true` so DB mirror failures fail closed instead of creating file-only accepted submissions, review decisions, revocation tombstones/status/audit rows, credit events, credit settlement control-plane rows, export provenance, or audit/content-read rows.
...
- Propagate revocation and retention expiration to DB metadata before DB-first reads. Completed for submission status, tombstones, reasoned privileged revocation audit rows, object-ref invalidation, derived-record invalidation, vector-entry invalidation, replay export manifest/item invalidation, file-backed benchmark/ranker provenance invalidation, hash-only process-evaluation/ranking-training worker queue invalidation rows, contributor credit/status reads, reviewer metadata reads, maintenance repair of already file-marked revoked submissions, retention-expired submission/object/derived/export invalidation, audit events for invalidation counts, and durable retention job/item ledger rows for maintenance runs.
...
- Tenant-isolation tests seed duplicate `submission_id`, `trace_id`, `canonical_summary_hash`, and contributor pseudonym under two tenants and prove all public store methods filter by tenant. Implemented for the PostgreSQL store facade and ingest DB mirror path, including raw PostgreSQL RLS visibility coverage for tenant rows, tenant policies, tenant grants, same-id submission/object/derived/vector/export rows, audit rows, credit and NEAR settlement control-plane rows, ranking/benchmark control-plane rows, tombstones, retention jobs/items, and revocation propagation rows when the test runtime role is RLS-safe, plus rejection coverage for export manifest items and derived records that try to link cross-tenant object, derived, or vector refs.
...
- Security tests verify PostgreSQL RLS with `trace-commons.trace_tenant_id` across tenants for both tenant metadata and corpus/control-plane rows, and a non-DB migration-registry regression test keeps the diagnostics table list aligned with central policy and FORCE RLS migration coverage.
...
- After revocation, status sync reports revoked, review approval fails, credit already earned and settled stays earned, and dataset export excludes the source.

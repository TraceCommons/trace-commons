...
- Ranking worker-run rows now include lifecycle status, completion timestamps, and optional hashed error details while keeping operator reason text hashed. Live non-dry-run ranking prediction-credit and model-promotion schedulers reject overlapping active non-stale rows for the same tenant and broad/narrow matching filters before appending a new `running` row, while admins can append-finalize stale running rows as `failed` through `/v1/admin/ranking/worker-runs/{ranking_worker_run_id}/recover-stale` without storing raw recovery notes.

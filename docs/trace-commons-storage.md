...
- Live credit settlement re-checks that central issuer profile before writing settlement batches or NEAR outbox rows; dry-runs and drills remain diagnostic surfaces instead of mutating fallbacks.
- Live credit-cycle worker runs also check the central issuer profile before claiming work, and scheduler preflight reports `credit_settlement_central_issuer_profile_incomplete` as a skip reason rather than starting partial cycle side effects.

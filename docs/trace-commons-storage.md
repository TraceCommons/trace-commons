...
- Admin-token config inspection through `/v1/admin/config-status`, returning only safe schema, DB cutover, object-primary, guardrail, max export item cap, submission quota, legal hold, ranking calibration freshness, label-source diversity, worker-run stale-threshold, NEAR submitter readiness/timeout, NEAR outbox submit bounds, credit-cycle step count, credit-cycle scheduler bounds/preflight support, object-store provider alias, object-store I/O enabled, and object-primary object-store eligibility fields with a read audit event.
...
- Ranking production policy includes `TRACE_COMMONS_RANKING_MIN_LABEL_COUNT`, `TRACE_COMMONS_RANKING_MIN_CONFIDENCE_THRESHOLD`, `TRACE_COMMONS_RANKING_MAX_AVERAGE_ABSOLUTE_ERROR_MICROS`, `TRACE_COMMONS_RANKING_MIN_PAIRWISE_LABEL_COUNT`, and `TRACE_COMMONS_RANKING_MIN_PAIRWISE_ACCURACY_MICROS`, server-owned quality gates applied after worker request parsing and model-risk recomputation so calibration workers cannot reduce the deployment's minimum sample-size, ordering-accuracy, or quality requirements.
...
- Ranking credit: set `TRACE_COMMONS_RANKING_CALIBRATION_MAX_AGE_HOURS`, `TRACE_COMMONS_RANKING_MIN_LABEL_COUNT`, `TRACE_COMMONS_RANKING_MIN_CONFIDENCE_THRESHOLD`, `TRACE_COMMONS_RANKING_MAX_AVERAGE_ABSOLUTE_ERROR_MICROS`, `TRACE_COMMONS_RANKING_MIN_LABEL_SOURCE_COUNT`, `TRACE_COMMONS_RANKING_MIN_PAIRWISE_LABEL_COUNT`, and `TRACE_COMMONS_RANKING_MIN_PAIRWISE_ACCURACY_MICROS` for production issuers, then verify stale calibration, insufficient joined-label sample size, low-confidence predictions, excessive aggregate/per-source error, insufficient label-source diversity, insufficient pairwise evidence, weak pairwise ordering accuracy, and a bad per-source calibration cohort block model promotion, prediction-credit issuance, and ranking-utility settlement before NEAR outbox submission is enabled.

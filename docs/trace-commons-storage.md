...
- Optional encrypted local artifact storage behind `TRACE_COMMONS_ARTIFACT_KEY_HEX`, with `TRACE_COMMONS_OBJECT_STORE=local_service` selecting the service-owned local encrypted backend used for production-shaped object refs. `TRACE_COMMONS_OBJECT_STORE_REQUIRE_VERSIONING=true` can fail startup unless the configured service-owned object store reports both object versioning and restore-after-delete support; current local-service and filesystem-remote adapters report false until a production provider with rollback semantics is compiled.
...
- Set `TRACE_COMMONS_OBJECT_STORE_REQUIRE_VERSIONING=true` only for a cloud-cutover candidate that must be rollback-complete at startup. Config status, operational summary, promotion gates, and operational metrics expose the versioning-required, object-versioning-supported, and restore-after-delete-supported booleans without object keys, bucket paths, or credentials. Local-service and filesystem-remote adapters intentionally fail this guard because they cannot prove object-version restore semantics.
- Run `POST /v1/admin/object-store-migration-drill` with `record_evidence: true` after configuring a service-owned object store. A green response has `ready: true`, no `blocking_gaps`, successful encrypted probe write/read/delete booleans, a sha256-prefixed probe object-ref hash, a sha256-prefixed migration manifest hash, and a recorded `object_store_migration` evidence row whose evidence ref points to that manifest hash without returning object keys, filesystem paths, credentials, or the probe payload. Set `require_versioning: true` for cloud-cutover rehearsal; local-service and filesystem-remote adapters currently return `object_versioning_unsupported` plus `restore_after_delete_unsupported` instead of claiming restore semantics they do not provide. After a versioned provider is compiled, pair this drill with `TRACE_COMMONS_OBJECT_STORE_REQUIRE_VERSIONING=true` so both runtime startup and drill evidence enforce the same rollback capability.
...
- Object-store migration: run `POST /v1/admin/object-store-migration-drill` with `record_evidence: true` after selecting `local_service` or enabled filesystem `remote_service` object storage. A green response has `ready: true`, the service-owned object-store alias, successful write/read/delete probe booleans, no raw object key or probe payload, a hash-only migration manifest, and a recorded `object_store_migration` evidence row that references the manifest hash. For cloud promotion rehearsals, repeat with `require_versioning: true` and require versioning/restore support to be true; then enable `TRACE_COMMONS_OBJECT_STORE_REQUIRE_VERSIONING=true` in the same canary environment and confirm config status, operational summary, promotion gates, and metrics all show required and supported restore semantics. Disabled cloud `remote_service` providers should fail this drill with safe blocker codes until a production provider is compiled.

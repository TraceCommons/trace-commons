...
- Live credit settlement re-checks that central issuer profile before writing settlement batches or NEAR outbox rows; when `TRACE_COMMONS_CREDIT_SETTLEMENT_CENTRAL_ISSUER_PRINCIPAL_REFS` is configured, only listed hashed principals can record issuer approvals, create or release credit holds, finalize live settlement, run live settlement/credit-cycle schedulers, run live NEAR credit outbox submit/confirm workers or schedulers, manually mark NEAR outbox status, or create credit-hold NEAR account freeze/unfreeze transitions. When `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_ROLLOUT_SMOKE_READY=true`, live settlement also requires fresh green rollout-smoke readiness before any live repair/write path. Dry-runs, drills, credit-cycle preflights, and NEAR outbox dry-runs remain diagnostic surfaces instead of mutating fallbacks.
...
maintenance, whose dedicated worker surface rejects ordinary reviewer tokens before DB/vector adapter checks. `process_eval_worker` is limited to writing bounded process-evaluation

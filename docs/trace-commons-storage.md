...
- Credit settlement: set `TRACE_COMMONS_CREDIT_SETTLEMENT_MAX_POINTS_PER_ACCOUNT` for production issuers, dry-run a tenant with both under-cap and over-cap pending credit, and verify over-cap accounts report `account_settlement_amount_exceeds_cap` without writing settlement batches or NEAR outbox rows.

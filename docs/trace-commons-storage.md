...
- Credit-settlement readiness in the same summary/metrics also exposes only booleans for the configured-contract requirement, configured-contract presence, per-account cap, policy-version allowlist configuration and missing-allowlist blocker state for positive delayed credit, submitter/confirmer adapters, central issuer approval gate, central issuer profile readiness, managed EdDSA enforcement, and tenant access-grant enforcement, plus a safe missing-control count for the central issuer profile. Config status additionally exposes whether live settlement is configured to require rollout-smoke readiness and how many hashed central issuer principal refs are pinned.

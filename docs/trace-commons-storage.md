...
- The credit-settlement drill accepts the same bounded `source_event_limit` shape as worker and scheduler settlement runs. Operators can therefore rehearse a partial batch, review the exact returned `source_list_hash`, record a hash-only issuer approval for that batch, and then finalize the matching worker/scheduler run without approving the whole backlog.
...
- When issuer approval is required for a bounded live settlement, run `POST /v1/admin/credit-settlement-drill` with the same `source_event_limit` as the worker or settlement scheduler, record the returned `source_list_hash` through `POST /v1/admin/credit-settlement-approvals`, and use the same approval hash on the live worker/scheduler request. Omit `source_event_limit` only when the approval is meant to cover the whole current backlog.

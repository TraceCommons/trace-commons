...
- Admin-token operational inspection through `/v1/admin/operational-summary` and `ironclaw traces operational-summary`, returning only safe tenant-scoped aggregate counts for submission status/risk, review SLA pressure, DB export manifests/jobs, retention jobs, vector coverage, benchmark lifecycle readiness plus external registry adapter gaps, ranking model/credit readiness including backtest pass/fail counts and reason totals, label-adjudication issue counts and reason totals, calibration-registry manifest-conflict blockers, ranking worker-run lifecycle health, and delayed credit totals with a read audit event.
...
- Admin ranking adjudication inspection through `/v1/admin/ranking/adjudication-report`, which groups latest absolute labels and pairwise preferences into safe issue counts for unresolved `disputed` labels, cross-source absolute-label outcome conflicts, and reversed pairwise preferences without exposing raw external refs. Operational summary projects those issue counts into ranking readiness and promotion-gate blockers.
...
- Model-promotion dry-runs now act as the candidate holdout-readiness preflight: responses include the registered calibration dataset hash, stored calibration report/joined-evidence hashes and counts, freshly recomputed current evidence hash/report hash/counts, effective server-owned thresholds, aggregate/per-source error metrics, low-confidence prediction count, promotability flag, and reason codes without exposing trace bodies or raw lab references. Promotion also requires the current model backtest to pass, so pairwise evidence or ordering failures block activation before a candidate can mint or settle ranking credit.
- Admin ranking model-backtest inspection through `/v1/admin/ranking/model-backtest-report`, which applies current calibration and pairwise preference checks to latest candidate and active model/target-use pairs, returns pass/fail counts, latest calibration hashes, joined-evidence/error metrics, pairwise accuracy, and the same machine-readable reason codes before promotion or credit issuance.

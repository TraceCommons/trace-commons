...
- Ranking credit: set `TRACE_COMMONS_RANKING_CALIBRATION_MAX_AGE_HOURS` and `TRACE_COMMONS_RANKING_MIN_LABEL_SOURCE_COUNT` for production issuers, then verify stale calibration, insufficient label-source diversity, and a bad per-source calibration cohort block model promotion, prediction-credit issuance, and ranking-utility settlement before NEAR outbox submission is enabled.

...
- Credit-settlement readiness in the same summary/metrics also exposes only booleans for the configured-contract requirement, configured-contract presence, per-account cap, submitter/confirmer adapters, central issuer approval gate, and central issuer profile readiness.

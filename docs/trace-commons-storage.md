...
- Ranking calibration production policy also includes `TRACE_COMMONS_RANKING_MIN_LABEL_COUNT`, a server-owned joined-label floor applied after worker request parsing so calibration workers cannot reduce the deployment's minimum sample-size requirement.

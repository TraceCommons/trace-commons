...
- Live credit settlement re-checks that central issuer profile before writing settlement batches or NEAR outbox rows; when `TRACE_COMMONS_CREDIT_SETTLEMENT_CENTRAL_ISSUER_PRINCIPAL_REFS` is configured, only listed hashed principals can record issuer approvals, finalize live settlement, or run live settlement/credit-cycle schedulers. When `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_ROLLOUT_SMOKE_READY=true`, live settlement also requires fresh green rollout-smoke readiness before any live repair/write path. Dry-runs, drills, and credit-cycle preflights remain diagnostic surfaces instead of mutating fallbacks.

...
- Object-store migration: run `POST /v1/admin/object-store-migration-drill` with `record_evidence: true` after selecting `local_service` or enabled filesystem `remote_service` object storage. A green response has `ready: true`, the service-owned object-store alias, successful write/read/delete probe booleans, no raw object key or probe payload, a hash-only migration manifest, and a recorded `object_store_migration` evidence row that references the manifest hash. For cloud promotion rehearsals, repeat with `require_versioning: true` and require versioning/restore support to be true; disabled cloud `remote_service` providers should fail this drill with safe blocker codes until a production provider is compiled.

...
- Tenant-isolation tests seed duplicate `submission_id`, `trace_id`, `canonical_summary_hash`, and contributor pseudonym under two tenants and prove all public store methods filter by tenant. Implemented for the PostgreSQL store facade and ingest DB mirror path, including raw PostgreSQL RLS visibility coverage for same-submission-id rows when the test runtime role is RLS-safe, plus rejection coverage for export manifest items and derived records that try to link cross-tenant object, derived, or vector refs.
...
- Security tests verify PostgreSQL RLS with `tracedao.trace_tenant_id` across tenants, and a non-DB migration-registry regression test keeps the diagnostics table list aligned with central policy and FORCE RLS migration coverage.

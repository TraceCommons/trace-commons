...
- Live credit-cycle worker runs also check the central issuer profile before claiming work, and scheduler preflight reports `credit_settlement_central_issuer_profile_incomplete` or `credit_settlement_rollout_smoke_not_ready` as skip reasons rather than starting partial cycle side effects.

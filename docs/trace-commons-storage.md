...
- Admin credit settlement, hold, utility-attestation, NEAR credit outbox, and benchmark registry outbox list reads append aggregate read-audit breadcrumbs with only the code-owned surface and bounded item count, using the same DB-mirrored audit helper as issuer approvals and risk summaries.

...
- Credit-settlement readiness in the same summary/metrics also exposes only booleans for the configured-contract requirement, configured-contract presence, per-account cap, policy-version allowlist, submitter/confirmer adapters, central issuer approval gate, central issuer profile readiness, managed EdDSA enforcement, and tenant access-grant enforcement, plus a safe missing-control count for the central issuer profile.

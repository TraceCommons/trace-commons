...
- Run retention as dry-run first through `/v1/admin/maintenance` or `/v1/workers/retention-maintenance`, with an explicit purpose and legal-hold policies configured. Pair it with `/v1/admin/db-reconciliation-drill` and `/v1/admin/audit-chain-drill` when promotion evidence is needed.

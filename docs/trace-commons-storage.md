...
- Admin ranking adjudication inspection through `/v1/admin/ranking/adjudication-report`, which groups latest absolute labels and pairwise preferences into safe issue counts for unresolved `disputed` labels, cross-source absolute-label outcome conflicts, and reversed pairwise preferences without exposing raw external refs. Backtest and active-model risk reports emit `ranking_adjudication_issues_present` for target uses with unresolved adjudication issues, so model promotion, prediction credit, settlement, and operational summary all see the same blocker.

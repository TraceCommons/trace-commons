...
- Analytics release: call `GET /v1/analytics/summary?release_scope=broad` after setting the production minimum cell count and `TRACE_COMMONS_ANALYTICS_BROAD_RELEASE_NOISE_KEY`; optionally set `TRACE_COMMONS_ANALYTICS_BROAD_RELEASE_NOISE_MAX_DELTA` to the approved count-noise bound. Verify missing noise config, suppressed cells, or disabled privacy budgets fail closed with safe reason codes before any operator republishes aggregates, then record the hash-only `analytics_release` rollout-smoke evidence row.

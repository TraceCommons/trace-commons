...
- Admin-token config inspection through `/v1/admin/config-status`, returning only safe schema, DB cutover, object-primary, guardrail, max export item cap, submission quota, legal hold, ranking calibration freshness, and object-store-provider status fields with a read audit event.
...
- Ranking credit: set `TRACE_COMMONS_RANKING_CALIBRATION_MAX_AGE_HOURS` for production issuers and verify stale calibration blocks model promotion, prediction-credit issuance, and ranking-utility settlement before NEAR outbox submission is enabled.

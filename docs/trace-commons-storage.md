...
- Credit-settlement readiness in the same summary/metrics also exposes only booleans for the configured-contract requirement, configured-contract presence, per-account cap, policy-version allowlist configuration and missing-allowlist blocker state for positive delayed credit, submitter/confirmer adapters, central issuer approval gate, central issuer profile readiness, managed EdDSA enforcement, and tenant access-grant enforcement, plus a safe missing-control count for the central issuer profile. Config status additionally exposes whether live settlement is configured to require rollout-smoke readiness.
...
- Live credit settlement re-checks that central issuer profile before writing settlement batches or NEAR outbox rows; when `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_ROLLOUT_SMOKE_READY=true`, it also requires fresh green rollout-smoke readiness before any live repair/write path. Dry-runs and drills remain diagnostic surfaces instead of mutating fallbacks.

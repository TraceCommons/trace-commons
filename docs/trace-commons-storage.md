...
- Optional fail-closed maintenance promotion gate behind `TRACE_COMMONS_REQUIRE_DB_RECONCILIATION_CLEAN=true`, which requires DB dual-write, rejects maintenance requests that omit `reconcile_db_mirror: true`, exposes compact `blocking_gaps`, turns DB/file reconciliation gaps into `409 Conflict` maintenance failures, treats DB canonical audit-payload projection drift as a named blocking gap, and only allows global or tenant-allowlisted DB reader promotion when `TRACE_COMMONS_REQUIRE_DB_MIRROR_WRITES=true` also makes mirror writes fail closed.

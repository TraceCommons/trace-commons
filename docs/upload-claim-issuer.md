...
`tracedao-upload-claim-issuer` is the standalone Ed25519 (EdDSA) issuer that
...
| `RUST_LOG` | `tracing-subscriber` filter | no | `tracedao_upload_claim_issuer=info,tracedao_server=info` |
...
1. **Generate a new keypair.**
   ...
   ```bash
   tracedao-upload-claim-issuer --generate-keypair > new-key.pem
   ```
...
```bash
cargo build --release --bin tracedao-upload-claim-issuer
./target/release/tracedao-upload-claim-issuer
```
...
```ini
[Service]
EnvironmentFile=/etc/tracedao/upload-claim-issuer.env
ExecStart=/usr/local/bin/tracedao-upload-claim-issuer
ExecStartPre=/usr/local/bin/tracedao-upload-claim-issuer --health-check
Restart=on-failure
KillSignal=SIGTERM
TimeoutStopSec=60
```
...
- `curl -fsS http://127.0.0.1:3917/.well-known/trace-commons-ed25519-keyset.json`
  ...
  `RSA` marker.
- `tracedao-upload-claim-issuer --health-check` — exits `0` on `OK`; `1` with
...
- `tracedao-upload-claim-issuer --mint-test-claim` — mints a test JWT against

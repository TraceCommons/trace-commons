# tracedao-ingest.rs size audit

Date: 2026-05-14
File: `crates/tracedao-server/src/bin/tracedao-ingest.rs`
...
The remaining production core (`AppState`, `app()` router, shared error/audit helpers like `sha256_prefixed`/`api_error`/`tenant_storage_ref`, signed-token verification stack, config parsing) stays in `tracedao-ingest.rs` or a sibling `mod state`/`mod auth` — call it ~15–18k of shared core.

Pulling the in-file `mod tests` out into `crates/tracedao-server/tests/` or a `#[path]`-included sibling is a separate ~60k-line move that is independently valuable and can be done first; it requires no behavioural understanding of the handlers.

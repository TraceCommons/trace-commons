...
| `TAIL_FRACTION_FLOOR_MICROS`          | **0** at launch      | Tail-fraction range showed real signal on Gemma 4 31B (0.20) but is uncalibrated against the pilot distribution. Calibrate post-first-1000-pilot-traces using only the tail-fraction column from `tracedao-gate-calibrate` output. |

...
Replace `MockVectorIndex` in `tracedao-gate-enclave` with a real
...
`crates/tracedao-gate-enclave/src/vector_index.rs`, gated by
...
| Env | Default | Notes |
|-----|---------|-------|
| `TRACE_COMMONS_VECTOR_INDEX_ROOT` | `/var/lib/tracedao-vector-index` | Filesystem root for per-tenant index files |
...
For the real impl, add unit tests inside `tracedao-gate-enclave`:

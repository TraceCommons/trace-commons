...
`bin/tracedao-ingest.rs`. There's a worker route + helper functions
that:
...
In `bin/tracedao-ingest.rs`, find the propagation dispatch (search
...
Add caller tests in `bin/tracedao-ingest.rs`:

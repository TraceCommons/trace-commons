...
trait (see `crates/trace-commons-protocol/src/trace_contribution.rs`). Today the
...
- Replacing the deterministic key-based redactor in
  `crates/trace-commons-protocol/src/redaction.rs`. The hosted classifier
...
2. If unset or empty → `Ok(None)` (deterministic redaction only,
   current behavior). The default `cargo check -p trace-commons-protocol`
...
- New module `crates/trace-commons-protocol/src/privacy_filter_near_ai.rs`
  behind a new Cargo feature `near-ai-privacy-filter`. Pattern mirrors the
  existing `near-ai-scorer` feature in `trace-commons-gate-enclave`.
...
**new direct deps on `trace-commons-protocol`** and need explicit approval
...
- **`reqwest` 0.12** as `optional = true, default-features = false,
  ...
  `trace-commons-gate-enclave/src/perplexity_near_ai.rs`. Required for the
  async HTTP client.
...
- Should the hand-rolled secret wrapper live in `trace-commons-protocol` or

...
Same crate, same file location as today: `crates/trace-commons-gate-enclave/
...
5. **Candle dep conflict.** Our crates currently git-pin candle to
   ...
   **Resolution:** **drop our direct candle deps** from
   `trace-commons-gate-enclave/Cargo.toml`. Let mistralrs supply candle
...
6. **Production-gate behavior parity test.** A2.2 doesn't have a
   test that exercises `trace-commons-ingest`'s scorer-init code path
...
1. **A2.3a — proof-of-concept** (~1 day). A tiny standalone binary
   under `crates/trace-commons-gate-enclave/examples/` that loads a model

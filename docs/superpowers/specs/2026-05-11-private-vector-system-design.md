...
This is the single slice that turns `tracedao-server` from "shippable in dev"
...
- **Unit (inside enclave):** the enclave code itself is a separate crate
  (probably `crates/tracedao-gate-enclave`). Standard Rust unit tests for
...
3. **Land `tracedao-gate-enclave` crate** as a separate binary. Inside-
...
- `tracedao-gate-enclave` crate (perplexity + embedder + index + gate

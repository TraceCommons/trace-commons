...
### What mining the toolchain established, and what it did not

A second pass tried to recover the schema from the tools themselves rather
than from the GUI, on the reasoning that a decoder usually names its keys.
Recorded here so nobody repeats it.

Where the vocabulary lives:

| Binary | Role | `*-specializations` |
| --- | --- | --- |
| `IconComposerFoundation` (inside Icon Composer.app) | the GUI's document model | present: `image-name-`, `fill-`, `opacity-`, `shadow-`, `position-`, `specular-`, `glass-`, `hidden-`, `blend-mode-`, `lighting-`, `translucency-`, `asset-mirroring-` |
| `AssetCatalogFoundation` | the binary that actually parses `icon.json` (it carries `automatic-gradient`, `groups`, `layers`, `fill`) | absent |
| `actool` | 92 KB shim | absent |
| `CoreThemeDefinition`, `IconFoundation`, `IconRendering` | compile and render side | absent |

The document model also yields `SpecializationSlot`, a Swift type whose static
members are exactly `base`, `dark` and `tinted` — so `dark` was the right slot
name, which makes the shape rather than the slot the open question.

**This is suggestive and it is not proof, for a reason worth writing down.**
`image-name` and `supported-platforms` are *also* absent from
`AssetCatalogFoundation`, and both demonstrably work: `image-name` is what
puts the named vector in the catalogue, verified by the dark SVG sitting
unreferenced in the same `Assets` directory and never appearing there. So
absence of a key from a binary does not establish that the key is unsupported,
and no conclusion of the form "the compiler does not implement specializations"
can be drawn from this table. What can be said is narrower: the exact
long-form keys the GUI writes do not appear in the compiler, and every shape
tried was ignored without a diagnostic.

Notably the compiler's parser does carry `appearance`, `dark`, `tinted` and
`value`, so an appearance mechanism plausibly exists there under a spelling
the GUI does not use. Guessing that spelling is the same unbounded loop as
before and was not attempted.

**What is needed, precisely.** One `icon.json` written by Icon Composer after
setting any dark override on any property — fill is easiest. The single
question it answers: does the override serialize as a `*-specializations` key,
or under some other spelling? Either answer ends this. A GUI-authored document
is not needed as an artifact and must not be committed; only its `icon.json`
text is wanted, and adopting whatever it shows is a one-line change here
because the document is generated.

...
1. **`CandlePerplexityScorer` is hardcoded to candle's Llama loader.**
   `crates/tracedao-gate-enclave/src/perplexity_candle.rs:142` imports
...
| Crate boundary | Add a small `BackendArch { Llama, Qwen3, Gemma3, Gemma4 }` enum **inside `tracedao-gate-enclave`** (the scorer's home crate). `CandidateArch` lives in `tracedao-server`'s bin module and **must not** become a dep of `tracedao-gate-enclave` — the dep direction is enclave ← server, not the reverse. Callers in `tracedao-server` convert their `CandidateArch` to `BackendArch` at the `try_new` call site. |

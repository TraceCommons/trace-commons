...
Wire `LocalPerplexityScorer` (in `crates/trace-commons-gate-enclave/src/perplexity_local.rs`)
...
- **Where to extend.** Implement the `TokenRarityScorer` trait on
  ...
  `crates/trace-commons-gate-enclave/src/perplexity_local.rs`. Keep the existing
...
- **Integration test.** Add a bake-off-binary integration test that runs
  ...
  lands.
- **Build hygiene.** Run `cargo test -p trace-commons-gate-enclave` and

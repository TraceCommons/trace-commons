...
| Eval framework | Extend existing `trace-commons-gate-calibrate` binary |
...
### Surface change in `trace-commons-gate-calibrate`

The existing offline binary at `crates/trace-commons-server/src/bin/
trace-commons-gate-calibrate.rs` runs perplexity calibration against a
...
```rust
// New CLI surface:
//   trace-commons-gate-calibrate bake-off \
...
1. `crates/trace-commons-server/src/bin/trace-commons-gate-calibrate.rs` extended
   with `--bake-off` mode.

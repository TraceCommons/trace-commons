...
Replace `MockEmbedder` in `trace-commons-gate-enclave` with a real
...
`crates/trace-commons-gate-enclave/src/embedder.rs`, gated by
...
| `TRACE_COMMONS_EMBEDDER_CACHE_DIR` | `/var/cache/trace-commons-embedder` | Model file cache |
...
`local-gpu-models` feature in `crates/trace-commons-gate-enclave/Cargo.toml`:
...
(`TRACE_COMMONS_EMBEDDER_INTEGRATION=1`). fastembed supports CPU inference

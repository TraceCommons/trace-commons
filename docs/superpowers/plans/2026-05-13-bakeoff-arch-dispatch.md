...
**Architecture:** A new `ScorerBackend` enum inside `crates/trace-commons-gate-enclave/src/perplexity_candle.rs` carries one of {`LlamaBackend`, `Qwen3Backend`, `Gemma3Backend`, `Gemma4Backend`}. Each holds the correctly-typed candle model + config + KV cache. A small `flatten_text_config` helper pre-processes multimodal `config.json` files. The public `CandlePerplexityScorer::score` interface is unchanged.

A second local enum `BackendArch { Llama, Qwen3, Gemma3, Gemma4 }` lives in `trace-commons-gate-enclave` and is the `try_new` parameter — **not** `CandidateArch`. The dep direction is enclave ← server (the manifest in `trace-commons-server` calls into the scorer in `trace-commons-gate-enclave`), so the scorer must not depend on the manifest's enum. Server-side callers translate `CandidateArch → BackendArch` at the call site via a small inline `match`.
...
| Path | What changes |
|------|--------------|
| `crates/trace-commons-gate-enclave/Cargo.toml` | Bump candle deps from `0.10.2` to a git-pinned commit on `main`. Pin in `Cargo.lock`. |
| `crates/trace-commons-gate-enclave/src/perplexity_candle.rs` | Refactor: hardcoded Llama loader → `ScorerBackend` enum with arch dispatch. Add `BackendArch` enum + `flatten_text_config` helper + inline tests. ~250 lines of touched code. |
| `crates/trace-commons-server/src/bin/gate_calibrate/bakeoff_manifest.rs` | Add `Qwen3` and `Gemma4` variants to `CandidateArch`; keep `Qwen2` as deprecated alias for `Qwen3` with a `tracing::warn!` on parse. |
| `crates/trace-commons-server/src/bin/gate_calibrate/run_candidate_eval.rs` | Update `ctx_for` to cover new variants; add `CandidateArch → BackendArch` translation at scorer construction. |
| `crates/trace-commons-server/src/bin/trace-commons-gate-calibrate.rs` | Update **two** call sites of `CandlePerplexityScorer::try_new`: `Cmd::BakeOff` (passes per-candidate arch) and `Cmd::Calibrate` (reads `TRACE_COMMONS_PERPLEXITY_MODEL_ARCH` env, default `"llama"`). |
| `crates/trace-commons-server/src/bin/trace-commons-ingest.rs:~4274` | Production gate-service scorer wiring. Update the `try_new` call to pass a `BackendArch` derived from `TRACE_COMMONS_PERPLEXITY_MODEL_ARCH` (same env var as calibrate). |
| `crates/trace-commons-server/tests/bakeoff_manifest.rs` | New tests: `parses_qwen3_arch`, `parses_gemma4_arch`, `qwen2_alias_warns_and_resolves_to_qwen3`. |
| In-crate test at `crates/trace-commons-gate-enclave/src/perplexity_candle.rs:~577` | Existing `try_new` invocation in the test must pass a `BackendArch` (use `BackendArch::Llama` to keep test semantics). |
...
cargo test -p trace-commons-server --test bakeoff_manifest --test bakeoff_subcommand
```
...
**Files:**
- Modify: `crates/trace-commons-gate-enclave/src/perplexity_candle.rs` (add helper at top of `candle_impl` mod, and inline `#[cfg(test)] mod tests`)
...
In `crates/trace-commons-gate-enclave/src/perplexity_candle.rs`, near the
...
```bash
cargo test -p trace-commons-gate-enclave --features local-gpu-models 2>&1 | tail -10
```
...
```bash
git add crates/trace-commons-gate-enclave/src/perplexity_candle.rs
...
**Files:**
- Modify: `crates/trace-commons-gate-enclave/Cargo.toml`
...
- [ ] **Step 2: Update `crates/trace-commons-gate-enclave/Cargo.toml`.**
...
If `trace-commons-server`'s Cargo.toml also pins specific candle versions
...
```bash
git add crates/trace-commons-gate-enclave/Cargo.toml Cargo.lock
...
**Files:**
- Modify: `crates/trace-commons-gate-enclave/src/perplexity_candle.rs`
...
mod candle_impl {
    ...
    /// Locally-owned arch enum. Distinct from `trace-commons-server`'s
...
1. `crates/trace-commons-server/src/bin/trace-commons-gate-calibrate.rs`
   `run_bakeoff` (`Cmd::BakeOff`) — convert per-candidate
   `CandidateArch` to `BackendArch` (small inline `match`).
2. `crates/trace-commons-server/src/bin/trace-commons-gate-calibrate.rs`
   `run_calibrate` (`Cmd::Calibrate`) — read
   `TRACE_COMMONS_PERPLEXITY_MODEL_ARCH` (default `"llama"`),
   parse into `BackendArch`.
3. `crates/trace-commons-server/src/bin/trace-commons-ingest.rs` (~line 4274)
...
4. In-crate test at
   `crates/trace-commons-gate-enclave/src/perplexity_candle.rs:~577` —
...
```bash
cargo check -p trace-commons-server --bins --features local-gpu-models
cargo test -p trace-commons-server --features local-gpu-models 2>&1 | tail -20
```
...
**Files:**
- Modify: `crates/trace-commons-server/src/bin/gate_calibrate/bakeoff_manifest.rs`
- Modify: `crates/trace-commons-server/src/bin/gate_calibrate/run_candidate_eval.rs`
- Modify: `crates/trace-commons-server/tests/bakeoff_manifest.rs`
...
```bash
cargo build --release --bin trace-commons-gate-calibrate
...
- [ ] `cargo check -p trace-commons-server --bins` clean (default features).
- [ ] `cargo check -p trace-commons-server --bins --features local-gpu-models` clean.
- [ ] `cargo test -p trace-commons-server` green — all existing + 3 new tests.
- [ ] `cargo test -p trace-commons-gate-enclave --features local-gpu-models` green — 4 new `flatten_text_config` tests.

...
**Goal:** A new `tracedao-pilot-bootstrap` binary that pulls HF agent-traces datasets, translates rows to `tracedao-protocol` submission envelopes, and POSTs through the existing `/v1/submissions` ingest API at a configurable rate. Generates 30k+ realistic submissions for A2.5 floor calibration + Phase A.5 metric design + end-to-end pipeline validation.

**Architecture:** Single binary in `crates/tracedao-server/src/bin/tracedao-pilot-bootstrap.rs`. Reads HF datasets via JSONL session loading (parquet support was removed in PR #67 after the real swival/pi-mono/DeepSeek datasets turned out to ship JSONL only). Translates rows via per-dataset translators, sends submissions via `reqwest`. Labeling sidecar in local JSONL. Single-process, single-tenant; deterministic submission IDs for idempotency.
...
| Path | Responsibility |
|------|----------------|
| `crates/tracedao-server/src/bin/tracedao-pilot-bootstrap.rs` | The binary entry point — CLI parsing, top-level orchestration |
| `crates/tracedao-server/src/bin/pilot_bootstrap/mod.rs` | Module root re-exporting the submodules |
| `crates/tracedao-server/src/bin/pilot_bootstrap/hf_dataset.rs` | HF dataset reader — parquet shard discovery, streaming row iteration |
| `crates/tracedao-server/src/bin/pilot_bootstrap/translators.rs` | `Translator` trait + concrete translators (`SwivalTranslator`, `PiMonoTranslator`, `DeepSeekAgentTranslator`) |
| `crates/tracedao-server/src/bin/pilot_bootstrap/submitter.rs` | Rate-limited HTTP submitter — bucket, retry, idempotency check |
| `crates/tracedao-server/src/bin/pilot_bootstrap/sidecar.rs` | Labeling sidecar — append-only JSONL writer |
| `crates/tracedao-server/tests/pilot_bootstrap_translators.rs` | Per-translator unit tests with fixture rows |
...
| Path | What changes |
|------|--------------|
| `crates/tracedao-server/Cargo.toml` | Add `[[bin]]` entry for `tracedao-pilot-bootstrap`. Verify `hf-hub`, `arrow`, `reqwest`, `serde_json`, `clap` are all already in tree; surface missing deps for approval before adding. |
| `docs/trace-commons-roadmap.md` | A.6 entry under Phase A status block |

**Out of scope (do not touch)**

- The gate-service binary (`tracedao-ingest`) — harness submits *to* it; doesn't modify it
- `tracedao-protocol` crate — harness *uses* its envelope types; doesn't extend them
...
```bash
grep -E "^hf-hub|^arrow|^reqwest|^serde_json|^clap" crates/tracedao-server/Cargo.toml crates/tracedao-gate-enclave/Cargo.toml
```

Confirm `reqwest`, `serde_json`, `clap` are already direct deps. `hf-hub` is in `tracedao-gate-enclave` for the candle path — verify it's accessible from `tracedao-server` (workspace dep or add a direct entry). `arrow` is likely *not* in tree — **surface for approval before adding**; alternative is `parquet` crate (also new) or shelling out to `huggingface-cli` to download + use `serde_json` parsing for jsonl-format datasets.
...
**Files:**
- Create: `crates/tracedao-server/src/bin/tracedao-pilot-bootstrap.rs`
- Create: `crates/tracedao-server/src/bin/pilot_bootstrap/mod.rs`
- Modify: `crates/tracedao-server/Cargo.toml` (add `[[bin]]` entry)
...
path = "src/bin/tracedao-pilot-bootstrap.rs"
```
...
```bash
git commit -m "Add tracedao-pilot-bootstrap binary scaffolding"
```
...
**Files:**
- Create: `crates/tracedao-server/src/bin/pilot_bootstrap/hf_dataset.rs`
...
**Files:**
- Create: `crates/tracedao-server/src/bin/pilot_bootstrap/translators.rs`
- Create: `crates/tracedao-server/tests/pilot_bootstrap_translators.rs`
...
**Files:**
- Create: `crates/tracedao-server/src/bin/pilot_bootstrap/submitter.rs`
- Create: `crates/tracedao-server/src/bin/pilot_bootstrap/sidecar.rs`
- Modify: `crates/tracedao-server/src/bin/tracedao-pilot-bootstrap.rs` (wire the main loop)
...
If `wiremock` isn't in tree, surface for approval; alternative is using `axum`'s test server (already in tree via `tracedao-server` deps) for the mock.
...
**Files:**
- Modify: `crates/tracedao-server/src/bin/pilot_bootstrap/translators.rs`
- Modify: `crates/tracedao-server/tests/pilot_bootstrap_translators.rs`
...
- Prerequisites: running `tracedao-ingest`, bootstrap-tenant token configured, HF cache space
...
- [ ] `cargo check -p tracedao-server --bins` clean (default + `--features local-gpu-models`)
- [ ] `cargo test -p tracedao-server --test pilot_bootstrap_translators` green
- [ ] `cargo test -p tracedao-server` overall green (existing + new)
- [ ] Six commits on `feat/a6-pilot-bootstrap`, subjects matching the plan:
  1. `Add tracedao-pilot-bootstrap binary scaffolding`
...
- **Does not modify the gate-service or `tracedao-ingest` binary.** The harness uses the existing API surface.
...
3. **v1 envelope contract.** Confirm with `tracedao-protocol` maintainer before Slice 4. If multi-part required, translator changes happen in Slice 3 + 5.

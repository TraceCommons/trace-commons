...
**Architecture:** Single binary in `crates/tracedao-server/src/bin/tracedao-pilot-bootstrap.rs`. Reads HF datasets via the `hf-hub` crate (already in tree for the candle path), parses parquet via `arrow`, translates rows via per-dataset translators, sends submissions via `reqwest`. Labeling sidecar in local JSONL. Single-process, single-tenant; deterministic submission IDs for idempotency.

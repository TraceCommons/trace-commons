...
Native rendering checkpoint: #945's reviewed synthetic-store render test shows the selected-store location and populated task/specification rows without app startup services. Against the newer exact-comparison FFI, its task-row wording assertion fails while the rows remain visible; that integration regression is under investigation. Do not transfer the earlier rendering pass to this newer build or claim interactive/human-pilot completion.

Production integration is local on an isolated stack at `1d02fc98`, with Astra clearance for non-activating code. The shared production arithmetic/oracle and legacy schema-1 re-projection checks passed. Strict Rust/Swift decoding rejects ambiguous nulls and unknown nested fields; targeted tests isolate the task cap. The immutable FFI built from source `30b960cf` passed 31 focused Swift tests on integrated Swift source `3fcc5c60`, including bridge/model, routing, and shell-wording coverage. Actual CLI and FFI saved-spec evaluation passed for the insufficient-support path. Remaining gates are the rendering regression, a supported nonempty synthetic two-cohort lifecycle through CLI/FFI, final integrated validation, and publication. Enabling new qualified specifications remains a separate change; no human pilot or model advantage is established.

...
| macOS comparison corrections and pilot store | [#944](https://github.com/TraceCommons/trace-commons/pull/944) fixes local Gregorian dates and saved-record identity/digest reconciliation. Explicit immutable store routing is in [#945](https://github.com/TraceCommons/trace-commons/pull/945), from reviewed commit `f44e7bc3`. Astra cleared both changes; focused Swift tests passed 25/25. Actual app pilot remains required; see the CI and routing follow-up checkpoint below. |

CI checkpoint: #942 at `a01efd88`, #943 at `d7636fcf`, and #944 at `f1a339f3` each passed all 26 checks. Real-FFI routing tests subsequently exposed concurrent native store-read contention; a reviewed shared actor gate fixes it, and the combined focused suite passed 27/27. The actor and shared-copy follow-ups are now published in #945 at `15f66a62`. The earlier `b9501e56` head failed the shell-wording baseline; the correction moves all seven routing messages to stateless Rust copy without widening that baseline or resolving any store. Astra cleared both fixes. Focused copy/header/ABI checks and 12 Swift tests passed; the updated-head CI and full local Clippy run remain pending. These checks do not replace the real two-cohort human pilot.

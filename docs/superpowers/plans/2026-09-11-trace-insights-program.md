...
CI checkpoint: #942 at `a01efd88`, #943 at `d7636fcf`, and #944 at `f1a339f3` each passed all 26 checks. Real-FFI routing tests subsequently exposed concurrent native store-read contention; a reviewed shared actor gate fixes it, and the combined focused suite passed 27/27. The actor and shared-copy follow-ups are now published in #945 at `15f66a62`. The earlier `b9501e56` head failed the shell-wording baseline; the correction moves all seven routing messages to stateless Rust copy without widening that baseline or resolving any store. Astra cleared both fixes. Focused copy/header/ABI checks and 12 Swift tests passed; full warning-denied contributor/FFI all-target Clippy passed with the existing allowances. At head `15f66a62`, 24 of 26 CI checks passed, including macOS app tests; two remained running with no failures at this checkpoint. These checks do not replace the real two-cohort human pilot.


Native rendering checkpoint: Sol implemented direct `InsightsView` hosting against a synthetic disposable store and the real FFI. Astra cleared local test commit `02de243a` after assertions were strengthened to require populated task and saved-specification rows and the selected-store location. The render test passed; this follow-up is published in #945 at `9ac49116`, with new-head CI pending. It verifies rendering without app startup services, not interaction coverage or a human pilot.

Production integration is in progress in an isolated worktree. Sol reports 13/13 comparison-specification tests and 6/6 arithmetic/oracle tests passing against the shared production primitive, including schema-1 baseline re-projection and byte/digest preservation. Schema-2 validation and CLI/macOS presentation are implemented locally. Remaining gates are complete CLI/FFI lifecycle and Swift adversarial/wording tests, integration of #945 routing fixes, Astra final review, and publication. Enabling new qualified specifications remains a separate change after these checks; local implementation is not admission.

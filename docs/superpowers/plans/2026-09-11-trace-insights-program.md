...
CI checkpoint: #942 at `a01efd88`, #943 at `d7636fcf`, and #944 at `f1a339f3` each passed all 26 checks. Real-FFI routing tests subsequently exposed concurrent native store-read contention; a reviewed shared actor gate fixes it, and the combined focused suite passed 27/27. That follow-up is awaiting publication with a shared-copy correction: #945 at `b9501e56` failed the macOS shell-wording baseline because its new messages were authored in Swift. Move those messages to shared Rust copy without widening the baseline. These checks do not replace the real two-cohort human pilot.

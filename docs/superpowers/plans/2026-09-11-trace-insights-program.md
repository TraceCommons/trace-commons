...
| Native Claude agent-branch attribution | [#941](https://github.com/TraceCommons/trace-commons/pull/941), stacked on #940. Astra cleared commit `1744f822`; real-source, malformed-input, and copied-store acceptance checks passed. Local Rust, Clippy, GTK, and macOS checks passed; remote CI at follow-up `8d018dc1` remains pending. Two selected branches supply source attribution; the third stays unavailable, and shared-session overlap prevents independent-task admission. |

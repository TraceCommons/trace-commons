...
- The bake-off binary (`trace-commons-gate-calibrate`) and its tests.
- The gate service code (`trace-commons-ingest`, `trace-commons-gate-enclave`).
...
- **Phase 1 (Offline HF bootstrap):** Update the floor calibration guidance:
  ...
  - `TAIL_FRACTION_FLOOR_MICROS`: ship at 0 for pilot launch. After the first ~1000 real pilot traces accumulate, calibrate per the existing Phase 1 procedure but using only the tail-fraction column from `trace-commons-gate-calibrate` output. The aggregate-perplexity column is misleading.
...
- [ ] `cargo check -p trace-commons-server --bins` clean (unchanged from baseline — no code changes).

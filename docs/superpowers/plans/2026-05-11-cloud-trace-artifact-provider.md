...
| Path | Responsibility |
|------|----------------|
| `crates/trace-commons-server/src/trace_artifact_kek.rs` | `KmsKeyWrapper` trait, `KekContext`, `WrappedDek`, `LocalMasterKeyWrapper` |
| `crates/trace-commons-server/src/trace_artifact_gcs.rs` | `GcsObjectClient` trait, `InMemoryGcsObjectClient`, `GcsRemoteTraceArtifactProvider` |
| `crates/trace-commons-server/src/trace_artifact_migration.rs` | Migration page+copy logic shared between worker handlers |
| `crates/trace-commons-server/tests/trace_artifact_kek.rs` | KEK contract tests |
| `crates/trace-commons-server/tests/trace_artifact_gcs.rs` | GCS provider contract tests against in-memory + (opt-in) fake-gcs-server |
| `crates/trace-commons-server/tests/trace_artifact_migration.rs` | Migration end-to-end tests |

**Modified files**

| Path | What changes |
|------|--------------|
| `crates/trace-commons-server/src/lib.rs` | Add `pub mod trace_artifact_kek; pub mod trace_artifact_gcs; pub mod trace_artifact_migration;` |
| `crates/trace-commons-server/src/trace_artifact_store.rs` | Extend `EncryptedTraceArtifact` with optional `wrapped_dek`; add schema_version "v2"; `verify_encrypted_artifact` gains context-hash match; `ServiceOwnedTraceArtifactStore` accepts a `KmsKeyWrapper`; legacy reader path preserved |
| `crates/trace-commons-server/src/bin/trace-commons-ingest.rs` | `TraceRemoteObjectStoreConfig` adds region/endpoint; `gcs` provider becomes constructable; new `/v1/workers/object-store/migrate{,/cleanup,/revert}` routes; drill reports KEK fields; rollout-smoke gains `object_store_kek_production_trust_boundary` check; startup gate `TRACE_COMMONS_KEK_REQUIRE_PRODUCTION_TRUST_BOUNDARY` |
...
**Files:**
- Create: `crates/trace-commons-server/src/trace_artifact_kek.rs`
- Modify: `crates/trace-commons-server/src/lib.rs` (add `pub mod trace_artifact_kek;`)
- Test: `crates/trace-commons-server/tests/trace_artifact_kek.rs` (created in Task 2)
...
```bash
cargo check -p trace-commons-server
```
...
```bash
git add crates/trace-commons-server/src/trace_artifact_kek.rs crates/trace-commons-server/src/lib.rs
...
**Files:**
- Modify: `crates/trace-commons-server/src/trace_artifact_kek.rs`
- Create: `crates/trace-commons-server/tests/trace_artifact_kek.rs`

- [ ] **Step 1: Write failing test** in `crates/trace-commons-server/tests/trace_artifact_kek.rs`
...
```bash
cargo test -p trace-commons-server --test trace_artifact_kek -- local_master_key_wrapper_round_trips_dek
```
...
```bash
git add crates/trace-commons-server/src/trace_artifact_kek.rs crates/trace-commons-server/tests/trace_artifact_kek.rs
...
**Files:**
- Modify: `crates/trace-commons-server/tests/trace_artifact_kek.rs`
...
```bash
cargo test -p trace-commons-server --test trace_artifact_kek
```

- [ ] **Step 3: Commit**

```bash
git add crates/trace-commons-server/tests/trace_artifact_kek.rs
...
**Files:**
- Modify: `crates/trace-commons-server/src/trace_artifact_store.rs`
...
```bash
git add crates/trace-commons-server/src/trace_artifact_store.rs
...
```bash
grep -rn "ServiceOwnedTraceArtifactStore::new\|ServiceOwnedTraceArtifactStore {" crates/trace-commons-server/src
```
...
**Files:**
- Create: `crates/trace-commons-server/src/trace_artifact_gcs.rs`
...
```bash
git add crates/trace-commons-server/src/trace_artifact_gcs.rs crates/trace-commons-server/src/lib.rs
...
**Files:**
- Modify: `crates/trace-commons-server/src/trace_artifact_gcs.rs`
- Create: `crates/trace-commons-server/tests/trace_artifact_gcs.rs`
...
```bash
git add crates/trace-commons-server/src/trace_artifact_gcs.rs crates/trace-commons-server/tests/trace_artifact_gcs.rs
...
**Files:**
- Modify: `crates/trace-commons-server/src/trace_artifact_gcs.rs`
- Modify: `crates/trace-commons-server/tests/trace_artifact_gcs.rs`
...
**Files:**
- Modify: `Cargo.toml` (workspace) and `crates/trace-commons-server/Cargo.toml`
- Modify: `crates/trace-commons-server/src/trace_artifact_gcs.rs`
...
```bash
cargo check -p trace-commons-server --bins
cargo check -p trace-commons-server --bins --features gcs-client
```

- [ ] **Step 5: Commit**

```bash
git add Cargo.toml crates/trace-commons-server/Cargo.toml crates/trace-commons-server/src/trace_artifact_gcs.rs
...
**Files:**
- Create: `crates/trace-commons-server/tests/trace_artifact_gcs_integration.rs`
...
```bash
GCS_FAKE_ENDPOINT=http://localhost:4443 cargo test -p trace-commons-server --features gcs-client --test trace_artifact_gcs_integration -- --ignored
```
...
```bash
git add crates/trace-commons-server/tests/trace_artifact_gcs_integration.rs docs/trace-commons-storage.md
...
**Files:**
- Create: `crates/trace-commons-server/src/trace_artifact_migration.rs`
- Modify: `crates/trace-commons-server/src/lib.rs`
...
```bash
git add crates/trace-commons-server/src/trace_artifact_migration.rs crates/trace-commons-server/src/lib.rs
...
**Files:**
- Modify: `crates/trace-commons-server/src/trace_artifact_migration.rs`
- Modify: `crates/trace-commons-server/tests/trace_artifact_migration.rs`
...
```bash
git add crates/trace-commons-server/src/trace_artifact_migration.rs crates/trace-commons-server/tests/trace_artifact_migration.rs
...
**Files:**
- Modify: `crates/trace-commons-server/src/db/trace_corpus_pg.rs`
- Modify: `crates/trace-commons-server/src/trace_corpus_storage.rs`
- Modify: `crates/trace-commons-server/tests/trace_corpus_storage_contract.rs` (or wherever the corpus contract test lives)
...
- [ ] **Step 1: Add a store method** `update_object_ref_store_alias(tenant_id, object_ref_id, expected_from_alias, to_alias) -> bool`. Atomic UPDATE with a WHERE clause that checks the current alias; returns whether one row was updated.
  ...
  ⚠️ Per `~/.claude/CLAUDE.md`: verify the new method compiles cleanly under **both** the postgres and libsql build configurations. Run `cargo check -p trace-commons-server` against each feature configuration before committing.
...
```bash
git add crates/trace-commons-server/src/db/ crates/trace-commons-server/src/trace_corpus_storage.rs crates/trace-commons-server/tests/
...
**Files:**
- Modify: `crates/trace-commons-server/src/bin/trace-commons-ingest.rs`
- Modify: `crates/trace-commons-server/src/trace_corpus_storage.rs` (if audit shape lives there)
...
```bash
cargo build -p trace-commons-server --release --bin trace-commons-ingest
cargo build -p trace-commons-server --release --bin trace-commons-upload-claim-issuer
```

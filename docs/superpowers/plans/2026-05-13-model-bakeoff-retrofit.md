...
| Path | Responsibility |
|------|----------------|
| `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_corpus.rs` | Tarball loader + slice validation |
| `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_metrics.rs` | AUC, paraphrase delta, tail-fraction range, determinism, throughput, VRAM accounting |
| `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_report.rs` | Decision-rule application, JSON + markdown emission |
| `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_manifest.rs` | `candidates.toml` parser |
| `crates/tracedao-server/tests/bakeoff_metrics.rs` | Metric unit tests with synthetic logprob fixtures |
| `crates/tracedao-server/tests/bakeoff_report.rs` | Decision-rule unit tests |
| `crates/tracedao-server/tests/bakeoff_manifest.rs` | Manifest parser tests |
...
| Path | What changes |
|------|--------------|
| `crates/tracedao-server/src/bin/tracedao-gate-calibrate.rs` | Add `bake-off` subcommand, plumb to the four modules above; existing `calibrate` mode untouched |
...
- The `calibrate` mode in `tracedao-gate-calibrate` — only `bake-off` is added.
...
**Files:**
- Create: `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_manifest.rs`
- Create: `crates/tracedao-server/tests/bakeoff_manifest.rs`
...
```bash
cargo test -p tracedao-server --test bakeoff_manifest 2>&1 | tail -10
```
...
```bash
git add crates/tracedao-server/src/bin/gate_calibrate/bakeoff_manifest.rs \
        crates/tracedao-server/tests/bakeoff_manifest.rs
...
**Files:**
- Create: `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_corpus.rs`
- Test: append to `crates/tracedao-server/tests/bakeoff_manifest.rs` (or new `bakeoff_corpus.rs`)
...
```rust
// crates/tracedao-server/tests/bakeoff_corpus.rs (new file)
...
```bash
git add crates/tracedao-server/src/bin/gate_calibrate/bakeoff_corpus.rs \
        crates/tracedao-server/tests/bakeoff_corpus.rs
...
**Files:**
- Create: `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_metrics.rs`
- Create: `crates/tracedao-server/tests/bakeoff_metrics.rs`
...
```bash
git add crates/tracedao-server/src/bin/gate_calibrate/bakeoff_metrics.rs \
        crates/tracedao-server/tests/bakeoff_metrics.rs
...
**Files:**
- Modify: `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_metrics.rs`
- Modify: `crates/tracedao-server/tests/bakeoff_metrics.rs`
...
**Files:**
- Create: `crates/tracedao-server/src/bin/gate_calibrate/bakeoff_report.rs`
- Create: `crates/tracedao-server/tests/bakeoff_report.rs`
...
> The existing `tracedao-gate-calibrate.rs` is env-driven with a flat
...
#[derive(Parser)]
#[command(name = "tracedao-gate-calibrate")]
struct Cli {
    #[command(subcommand)]
    cmd: Option<Cmd>,
}
...
`grep '^clap' crates/tracedao-server/Cargo.toml` — used elsewhere).
...
fn calibrate_subcommand_is_default_when_omitted() {
    let cli = Cli::parse_from(["tracedao-gate-calibrate"]);
    assert!(matches!(cli.cmd, None));
}

#[test]
fn calibrate_subcommand_parses_explicitly() {
    let cli = Cli::parse_from(["tracedao-gate-calibrate", "calibrate"]);
    assert!(matches!(cli.cmd, Some(Cmd::Calibrate)));
}
```

- [ ] **Step 4: Confirm `cargo check -p tracedao-server --bin tracedao-gate-calibrate` is clean and existing tests still pass.**

- [ ] **Step 5: Commit.**

```bash
git commit -m "Refactor tracedao-gate-calibrate to clap subcommand dispatch"
```
...
- [ ] **Step 3: Confirm `cargo check -p tracedao-server --bins --features local-gpu-models` is clean**

- [ ] **Step 4: Commit**

```bash
git commit -m "Wire bake-off subcommand into tracedao-gate-calibrate"
```
...
- [ ] **Step 5: Confirm `cargo test -p tracedao-server` is green.**
...
**Files:**
- Create: `crates/tracedao-server/src/bin/gate_calibrate/mock_scorer.rs`
- Modify: `crates/tracedao-gate-enclave/src/perplexity.rs` (verify the
  `PerplexityScorer` trait is `pub`; if not, expose it)
- Test: `crates/tracedao-server/tests/mock_scorer.rs`
...
- [ ] **Step 1: Confirm `PerplexityScorer` trait is `pub` in
  `tracedao-gate-enclave`. If not, expose it via an
...
- [ ] `cargo check -p tracedao-server --bins --features local-gpu-models` is clean.
- [ ] `cargo test -p tracedao-server --test bakeoff_manifest --test bakeoff_corpus --test bakeoff_metrics --test bakeoff_report` is green.
- [ ] `tracedao-gate-calibrate bake-off --help` prints reasonable usage.

...
Implementation checkpoint: draft [#947](https://github.com/TraceCommons/trace-commons/pull/947) at `d870dcbd` publishes the reviewed non-activating integration on #945. Production arithmetic/oracle and schema-1 re-projection checks passed. Focused native checks passed 31/31 using the current immutable FFI, including strict nested decoding and an otherwise consistent over-cap fixture. The reviewed rendering follow-up passed 1/1 with stable populated-row OCR assertions and an explicit backend-state check. Actual CLI/FFI evaluation now covers both insufficient support and a supported four-task/two-cohort synthetic store. Astra cleared that fixture and actual-call coverage; the full CLI suite passed 16/16 and FFI Insights suite 15/15. Actual supported evaluation through the macOS router/model now passes 1/1, with the full routing integration class 4/4 and Astra clearance at source `02d5816f`. Current-head CI remains required before publication of the separate admission change. Synthetic fixtures and native tests do not satisfy the two-cohort human pilot.

...
Native Claude import and declaration inspection are in drafts [#939](https://github.com/TraceCommons/trace-commons/pull/939) and [#940](https://github.com/TraceCommons/trace-commons/pull/940). Linux CI exposed a stale source-picker selection in the lifecycle fixture and an unhandled Claude declaration variant in GTK; both require fixes and verification. The next Claude attribution rule must address identified agent branches and context-bearing attachments because the selected real pilot files are swarm work. A top-level-only rule that excludes all three files cannot close that milestone. Native segment termination, user-assessed acceptance, complete task boundaries, and independent tasks remain distinct evidence requirements.

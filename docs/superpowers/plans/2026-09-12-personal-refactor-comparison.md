...
Implementation checkpoint: the isolated non-activating stack at `1d02fc98` has Astra code-review clearance. Production arithmetic/oracle and schema-1 re-projection checks passed. Focused native checks passed 31/31 using the current immutable FFI, including strict nested decoding and an otherwise consistent over-cap fixture. Actual CLI/FFI evaluation covers insufficient support; supported nonempty two-cohort evaluation remains required. A native render assertion fails against the newer FFI and is being investigated. Resolve that regression, complete the supported lifecycle and final integrated checks, and publish before the separate admission change. Synthetic fixtures and native tests do not satisfy the two-cohort human pilot.

...
Phase: A.6 follow-up. Verifies whether `tracedao-pilot-bootstrap` (PR #47)
...
- `cargo build --release -p tracedao-server --bin tracedao-pilot-bootstrap --bin tracedao-ingest` succeeds.
...
```bash
TRACE_COMMONS_PILOT_TENANT_TOKEN=dummy ./target/release/tracedao-pilot-bootstrap \
...
`crates/tracedao-server/src/bin/pilot_bootstrap/hf_dataset.rs` lines 86–99
...
```bash
cd /path/to/tracedao-server
cargo build --release -p tracedao-server --bin tracedao-pilot-bootstrap
...
./target/release/tracedao-pilot-bootstrap --source badlogicgames/pi-mono ...
# => Error: dataset badlogicgames/pi-mono exposes no .parquet shards
./target/release/tracedao-pilot-bootstrap --source TeichAI/DeepSeek-v4-Pro-Agent ...

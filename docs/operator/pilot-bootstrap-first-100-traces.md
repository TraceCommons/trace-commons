...
PR #67 made `trace-commons-pilot-bootstrap` real-data-capable against the
...
`trace-commons-ingest` to verify the gate decision distribution is sane
and that the audit chain row count matches the submission count.

## Goal

Submit 100 real swival traces through a staging `trace-commons-ingest`
deployment. Verify:
...
2. **Staging `trace-commons-ingest`** running with:
...
3. **Local `trace-commons-pilot-bootstrap` binary**:

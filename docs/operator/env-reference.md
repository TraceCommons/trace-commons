...
Every env var the `tracedao-ingest` binary reads, grouped by surface. The
master list is `grep '^const TRACE_COMMONS_' crates/tracedao-server/src/bin/tracedao-ingest.rs`.
...
| `TRACE_COMMONS_EMBEDDER_CACHE_DIR` | optional | `/var/cache/tracedao-embedder` | Where fastembed caches ONNX weights. |
...
| Var | R? | Default | Description |
|---|---|---|---|
| `TRACE_COMMONS_VECTOR_INDEX_ROOT` | optional | `/var/lib/tracedao-vector-index` | On-disk per-tenant index root. Must be on a fast local disk. |

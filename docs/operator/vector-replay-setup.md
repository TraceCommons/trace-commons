...
| `local-gpu-models` feature build | `cargo build -p trace-commons-server --bin trace-commons-vector-replay --features local-gpu-models`. The default-feature build does not include the binary at all. |

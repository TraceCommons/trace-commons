# `trace-commons-vector-replay` — Setup Requirements & Verification

This doc captures the prerequisites and fixture shape an operator needs in
order to drive `trace-commons-vector-replay` end-to-end against a controlled
...
>   `crates/trace-commons-server/tests/vector_replay_store_methods.rs`,
>   driven by `TRACE_COMMONS_PG_TEST_DATABASE_URL`.
...
| `local-gpu-models` feature build | Yes | `cargo build -p trace-commons-server --bin trace-commons-vector-replay --features local-gpu-models`. The default-feature build does not include the binary at all. |
...
- **GCP Cloud KMS not wired.** The async unwrap path used by
  `trace-commons-ingest` is not mirrored here. Operators on GCS-only
...
- [ ] `trace-commons-ingest` is stopped (or the target tenant is quiesced)

...
The `trace-commons-pilot-bootstrap` binary replays HuggingFace agent-traces
datasets into a running `trace-commons-ingest` so the pilot has real-shaped
...
1. A running `trace-commons-ingest` (locally or on a staging deployment).
...
TRACE_COMMONS_PILOT_TENANT_TOKEN=<token> \
  cargo run -p trace-commons-server --bin trace-commons-pilot-bootstrap -- \
...
```
TRACE_COMMONS_PILOT_TENANT_TOKEN=<token> \
  cargo run --release -p trace-commons-server --bin trace-commons-pilot-bootstrap -- \
    --source jedisct1/agent-traces-swival \
    --count 30000 \
    --target http://localhost:3907 \
    --rate 1 \

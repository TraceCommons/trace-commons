...
After participation begins, keep the participant grant on the account runtime that executes account merges. A merge involving reward identities fails closed when its grant is missing. The reward hook shares the account merge transaction: it requires the proposal consumed by that transaction, preserves every immutable participant hash, and moves ownership through aliases. A failed hook rolls back proposal consumption and the account merge. Accounts without reward mappings retain the existing merge behavior. The hook re-checks, over the joined alias set, the two invariants a reservation must satisfy: no program may exceed its `participant_cap_units` and no work namespace may be claimed twice. A merge that would breach either is refused with `reward_merge_participant_cap` or `reward_merge_work_duplicate`, so two accounts each holding the per-participant maximum cannot be combined into one identity holding twice it. The cap refusal clears once the excess reservations expire or reach a terminal state; the work-namespace refusal is for the lifetime of the reservations.
...
Errors use existing safe `reward_*` labels: invalid input is 400, missing account authentication is 401, foreign/unknown objects are 404, unavailable storage is 503, and an unprovisioned participant database login is `reward_participant_unprovisioned` at 503 rather than 401 so a missing DBA step is not mistaken for a bad credential, and state/capacity/version conflicts are 409. Cross-site writes return 403 and exhausted request limits return 429. Render actionable copy from the label; keep server/internal identifiers out of user-facing error prose.

Each ingest process admits at most two concurrent reward database queries across public and account routes, with at most one reservation per tenant and at most two authenticated reads per tenant. Reservations take the tenant's exclusive advisory lock and so must not overlap; status and history take only the shared lock, so they are admitted alongside an in-flight reservation and wait on that lock inside PostgreSQL rather than being refused in process. Anonymous reward offer, mission catalog and mission detail reads additionally share one public-read permit, so they can occupy at most one of the two database slots. Account operations retain the existing tenant and global budgets. Contending requests return 429 before waiting for a database connection.

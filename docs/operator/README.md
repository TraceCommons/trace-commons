# Operator Runbook — `tracedao-server`

This directory holds the deployment-shaped knowledge that lets an operator
take `tracedao-server` from "compiles clean on `main`" to "first pilot
...
|---|---|
| First-time deploying `tracedao-server` | [`./deployment.md`](./deployment.md) |
...
- Owns the GCP project (or other cloud) where `tracedao-server` will run
...
- **`vector-replay.md`** — operator reference for the
  `tracedao-vector-replay` recovery binary. Rebuilds a tenant's local

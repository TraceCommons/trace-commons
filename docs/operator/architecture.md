...
One-page picture of every box a `tracedao-server` pilot needs and how they
...
```
                  ...
                  +-------------------+--------------------+
                                      ...
                                      v
+-----------------------+   HTTPS   +------------------------------+
| tracedao-upload-      |<--------> | tracedao-ingest              |
...
| Component | In-process with `tracedao-ingest`? | Notes |
|---|---|---|
...
`tracedao-gate-enclave` crate's `EnclaveGateOrchestrator` trait surface —
...
`tracedao-ingest` and in the GPU box (which is the same process). Wrapped

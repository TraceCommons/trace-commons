...
- Retention purge promotion starts with dry runs, legal-hold review, durable retention job/item inspection, and audit-chain verification. Legal-hold skips are only trusted when the stored policy id matches the server-derived retention class for the submission's allowed uses. Destructive purge should stay behind tenant kill switches until a dry-run report, reconciliation report, and rollback plan have been reviewed.

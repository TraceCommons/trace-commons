...
- Centralized early issuer mode is now represented as explicit fail-closed controls rather than a broad actor trust model: `TRACE_COMMONS_CREDIT_SETTLEMENT_REQUIRE_CENTRAL_ISSUER_PROFILE=true` validates DB mirror/RLS, managed EdDSA tokens, tenant grants, exact hashed issuer principal refs, issuer approvals, caps, NEAR contract pinning, adapters, adapter auth, and rollout-smoke live-settlement gating at startup; issuer-approval, live-settlement, live NEAR outbox submit/confirm, manual NEAR outbox status, hold-driven NEAR account transitions, and live scheduler paths enforce that principal allowlist while preserving dry-run inspection, drills expose safe missing-control names, operational metrics expose safe central-profile gauges, live settlement re-checks the profile and rollout-smoke readiness before writing batches or NEAR outbox rows, live credit-cycle scheduler startup and route execution fail closed before worker claim when source-list approval is required, and credit-cycle/outbox preflight or dry-run paths stop before partial cycle side effects.
- Settlement drills now accept the same bounded `source_event_limit` as worker/scheduler settlement, so centrally operated issuers can approve an exact partial source list without approving the full backlog.
...
- Bounded source-list review is now part of the direct issuance path: a drill with `source_event_limit` returns the same canonical source-list hash that the worker or scheduler will require for live settlement approval.

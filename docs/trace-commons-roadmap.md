...
- Ranking-derived credit now has a bounded worker coordinator that runs calibration, model promotion, prediction-credit issuance, settlement, and NEAR outbox inspection/submission for one model/policy/target without granting broad admin settlement access.

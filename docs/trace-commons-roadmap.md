...
- Centrally run issuance can now pin approved settlement policy versions with `TRACE_COMMONS_CREDIT_SETTLEMENT_ALLOWED_POLICY_VERSIONS`; dry-runs and drills still expose source-list hashes for review, while live settlement rejects unlisted policy versions before writing settlement batches or NEAR outbox rows.

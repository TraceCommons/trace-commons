...
- [ ] Build runbooks and smoke checks for per-tenant rollout, rollback, key rotation, object-store migration, retention purge dry runs, and audit-chain verification. A first rehearsal-ready operator draft now lives in `docs/trace-commons-storage.md`, operational summary now exposes the required smoke-check names plus recorded/missing rehearsal evidence, and admins can append hash-only rehearsal evidence through `POST /v1/admin/rollout-smoke/evidence`; keep this unchecked until the smoke pass is rehearsed end to end.

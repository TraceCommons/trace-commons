...
- [ ] Promote DB/object-primary submit, review, replay, benchmark, and ranker paths from pilot flags to per-tenant rollout flags after reconciliation parity is green. Initial tenant allowlist gates now wrap DB contributor/reviewer/replay/audit/tenant-policy reads, object-ref-required modes, and object-primary submit/replay/derived flags; contributor-credit, object-primary submit/review, object-primary replay export, and benchmark/ranker derived export caller tests cover tenant A canary rollout while tenant B remains on file fallback.

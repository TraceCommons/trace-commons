...
- [ ] Build runbooks and smoke checks for per-tenant rollout, rollback, key rotation, object-store migration, retention purge dry runs, and audit-chain verification. A first rehearsal-ready operator draft now lives in `docs/trace-commons-storage.md`, operational summary now exposes the required smoke-check names including `tenant_canary_isolation`, `db_reconciliation_clean`, `rollback_flag_drill`, `key_rotation_drill`, retention dry-run, PostgreSQL RLS readiness, and audit-chain verification plus recorded/passed/failed/stale/missing rehearsal evidence, and admins can inspect or append hash-only rehearsal evidence through `GET`/`POST /v1/admin/rollout-smoke/evidence`, including `latest_only=true` for current per-check state. Dedicated admin drills can now record `db_reconciliation_clean`, `rollback_flag_drill`, `key_rotation_drill`, `retention_dry_run`, `postgres_rls_readiness`, and `audit_chain_verification` evidence without making operators synthesize those rows manually; keep this unchecked until the smoke pass is rehearsed end to end.
